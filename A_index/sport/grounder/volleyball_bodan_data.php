...
$sql		=	"select id from volleyball_match where Match_Type=10 and Match_IsLose=1 and Match_CoverDate>DATE_ADD(now(),INTERVAL -12 HOUR) ".$sqlwhere.' order by Match_CoverDate,match_name';
...
if($i == 1){ //未查找到记录
	...
	$sql	=	"select match_name from volleyball_match where Match_Type=10 and Match_CoverDate>DATE_ADD(now(),INTERVAL -12 HOUR) group by match_name";
	$query	=	$mysqli->query($sql);
	$i		=	0;
	$lsm	=	'';
	while($row = $query->fetch_array()){
