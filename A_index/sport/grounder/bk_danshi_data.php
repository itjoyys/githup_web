...
$sql		=	"select id from lq_match WHERE Match_Type!=3 and Match_IsLose=1 AND Match_CoverDate>DATE_ADD(now(),INTERVAL -12 HOUR) AND Match_Date='".date("m-d")."' ".$sqlwhere.'  order by Match_CoverDate,iPage,iSn,Match_ID,match_name,Match_Master';//print_r($sql);exit;
...
if($i == 1){ //未查找到记录
	...
	$sql	=	"select match_name from lq_match WHERE Match_Type!=3 and Match_IsLose=1 AND Match_CoverDate>DATE_ADD(now(),INTERVAL -12 HOUR) AND Match_Date='".date("m-d")."' group by match_name";
	$query	=	$mysqli->query($sql);
	$i		=	0;
	$lsm	=	'';
	while($row = $query->fetch_array()){
	...
	$query	=	$mysqli->query($sql);
	$i		=	0;
	while($rows = $query->fetch_array()){ //print_r($rows);exit;
