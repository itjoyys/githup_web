...
$sql		=	"select id from baseball_match WHERE Match_Type=1 and Match_IsLose=1 and Match_CoverDate>DATE_ADD(now(),INTERVAL -12 HOUR) and Match_Date='".date("m-d",strtotime("-12 hours"))."' ".$sqlwhere.' order by Match_CoverDate,match_name';//print_r($sql);exit;
