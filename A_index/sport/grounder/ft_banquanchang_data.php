...
$sql		=	"select id from bet_match WHERE Match_Date='".date("m-d",strtotime("-12 hours"))."' and Match_IsLose=1 and Match_CoverDate>DATE_ADD(now(),INTERVAL -12 HOUR) and Match_BqMM>0 ".$sqlwhere.' order by Match_CoverDate,iPage,match_name,Match_Master,Match_ID,iSn';
...
if($i == 1){ //未查找到记录
	...
	$sql	=	"select match_name from bet_match WHERE Match_Date='".date("m-d",strtotime("-12 hours"))."' and Match_CoverDate>DATE_ADD(now(),INTERVAL -12 HOUR) and Match_BqMM>0 group by match_name";
	$query	=	$mysqli->query($sql);
	$i		=	0;
	$lsm	=	'';
	while($row = $query->fetch_array()){
