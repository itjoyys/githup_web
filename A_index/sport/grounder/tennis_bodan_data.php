...
if($i == 1){ //未查找到记录
	...
	$sql	=	"select Match_Name from tennis_Match where Match_Type=10 and Match_IsLose=1 and Match_Bd21>0 and  Match_CoverDate>DATE_ADD(now(),INTERVAL -12 HOUR) group by match_name";
	$query	=	$mysqli->query($sql);
	$i		=	0;
	$lsm	=	'';
	while($row = $query->fetch_array()){
