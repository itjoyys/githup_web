...
$sql		=	"select id from volleyball_match WHERE Match_Type=1 and Match_IsLose=1 AND Match_CoverDate>DATE_ADD(now(),INTERVAL -12 HOUR) AND Match_Date='".date("m-d",strtotime("-12 hours"))."' ".$sqlwhere.' order by Match_CoverDate,match_name';//print_r($sql);exit;
...
if($i == 1){ //未查找到记录
	...
	$sql	=	"select match_name from volleyball_match WHERE Match_Type=1 AND Match_CoverDate>DATE_ADD(now(),INTERVAL -12 HOUR) AND Match_Date='".date("m-d",strtotime("-12 hours"))."' group by match_name";
	$query	=	$mysqli->query($sql);
	$i		=	0;
	$lsm	=	'';
	while($row = $query->fetch_array()){
