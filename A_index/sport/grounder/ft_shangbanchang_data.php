...
$sql		=	"select id from bet_match where Match_Type=1 and Match_IsLose=1 and match_date='".date("m-d",strtotime("-12 hours"))."' AND Match_CoverDate>DATE_ADD(now(),INTERVAL -12 HOUR) and (Match_BHo+Match_BAo<>0 or Match_Bdpl+Match_Bxpl<>0) and Match_IsShowb=1 ".$sqlwhere.' order by Match_CoverDate,iPage,match_name,Match_Master,Match_ID,iSn';
//print_r($sql);exit;
