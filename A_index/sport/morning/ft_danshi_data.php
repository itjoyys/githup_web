...
if($action=='c') $sql		=	"select id from bet_match WHERE Match_Type=0 and match_name not like '%特别投注%' AND Match_CoverDate>now() and match_date!='".date("m-d")."' ".$sqlwhere.' and match_bzg>0 order by Match_CoverDate,iPage,match_name,Match_Master,Match_ID,iSn';
else $sql		=	"select id from bet_match WHERE Match_Type=0 AND Match_CoverDate>now() and match_date!='".date("m-d")."' ".$sqlwhere.' order by Match_CoverDate,iPage,match_name,Match_Master,Match_ID,iSn';
