        ...
        <td>
                                                ...
                                                <tr id="shuaxin">
                                                    ...
                                                    <td  id="SortGame"  class="SortGame"  name="SortGame">
                                                        <select  id="ymd" name="ymd">
                                                            <?php
                                                            for($i=0;$i<7;$i++){
                                                                $d	=	date('Y-m-d',strtotime(date('Y-m-d'))-$i*86400);
                                                                ?>
                        ...
                        <table cellspacing="0"  cellpadding="0"  class="game">
                            ...
                            if(!$rows){
                                ...
                                do{
                                    ...
                                    ?>
                                    <tr  id="TR_01-<?=$rows["Match_ID"]?>"    *class*="">
				<td  rowspan="2"  class="b_cen" width="26%"><?=$rows["Match_MatchTime"]?></td>
				<td  rowspan="2"  class="b_cen"  width="26%"> <?=$rows["match_master"]?> <br>
                    <?=$rows["match_guest"]?> </td>
				<? if($rows["MB_Inball"]<0) {?>
                                        <td colspan=2 class="b_cen"  id="<?=$rows["Match_ID"]?>_MH">赛事无效</td>

                                    <?php }else{ ?>
                                        <td  class="b_cen"  id="<?=$rows["Match_ID"]?>_MH"><?=$rows["MB_Inball_HR"]?></td>
                                        <td  class="b_cen"  id="<?=$rows["Match_ID"]?>_PRH"><?=$rows["MB_Inball"]?></td>
                                    <?php } ?>
		  ...
		  <tr  id="TR_01-1870256"    *class*="">
				<? if($rows["TG_Inball"]<0) {?>
                                    ...
                                    <?php }else{ ?>
                                        <td  class="b_cen"  id="<?=$rows["Match_ID"]?>_MH"><?=$rows["TG_Inball_HR"] ?></td>
                                        <td  class="b_cen"  id="<?=$rows["Match_ID"]?>_PRH"><?=$rows["TG_Inball"]?></td>
                                    <?php } ?>
		  </tr>
	<?php
