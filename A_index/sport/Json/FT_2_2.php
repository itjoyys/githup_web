...
$sql		=	"select id from bet_match where Match_Type=0 and Match_IsShowt=1 AND Match_CoverDate>DATE_ADD(now(),INTERVAL -12 HOUR) and Match_DsDpl>0 ".$sqlwhere.' order by Match_CoverDate,iPage,match_name,Match_Master,Match_ID,iSn';
...
if($i == 1){ //未查找到记录
	...
	$sql	=	"select match_name from bet_match where Match_Type=0 and Match_IsShowt=1 AND Match_CoverDate>DATE_ADD(now(),INTERVAL -12 HOUR) and Match_DsDpl>0 group by match_name";
	$query	=	$mysqls->query($sql);
	$i		=	0;
	$lsm	=	'';
	while($row = $query->fetch_array()){
