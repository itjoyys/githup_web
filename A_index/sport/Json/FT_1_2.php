...
$sql		=	"select id from bet_match where Match_Type=1 and Match_IsShowt=1 and Match_DsDpl>0 AND Match_CoverDate>DATE_ADD(now(),INTERVAL -12 HOUR)".$sqlwhere.' order by Match_CoverDate,iPage,match_name,Match_Master,Match_ID,iSn';
...
if($i == 1){ //未查找到记录
	...
	$sql	=	"select match_name from bet_match where Match_Type=1 and Match_IsShowt=1 and Match_DsDpl>0 AND Match_CoverDate>DATE_ADD(now(),INTERVAL -12 HOUR) group by match_name";
	$query	=	$mysqls->query($sql);
	$i		=	0;
	$lsm	=	'';
	while($row = $query->fetch_array()){
