...
if($i == 1){ //未查找到记录
	...
	$sql	=	"select match_name from tennis_Match WHERE Match_Type=1 and Match_CoverDate>DATE_ADD(now(),INTERVAL -12 HOUR) AND Match_Date='".date("m-d",strtotime("-12 hours"))."' group by match_name";
	$query	=	$mysqls->query($sql);
	$i		=	0;
	$lsm	=	'';
	while($row = $query->fetch_array()){
