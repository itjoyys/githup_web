...
//$sql		=	"select id from bet_match WHERE Match_Type=1 AND Match_CoverDate>DATE_ADD(now(),INTERVAL -12 HOUR) AND Match_Date='".date("m-d",strtotime("-12 hours"))."' ".$sqlwhere.' order by Match_CoverDate,iPage,match_name,Match_Master,Match_ID,iSn';
if($action=='c')$sql		=	"select id from bet_match WHERE Match_Type=1 and match_name not like '%特别投注%' and match_bzg>0 AND Match_CoverDate>'".date('Y-m-d H:i:s')."' AND Match_Date='".date("m-d")."' and Match_HalfId is not null ".$sqlwhere.' order by Match_CoverDate,iPage,match_name,Match_Master,Match_ID,iSn';
else $sql		=	"select id from bet_match WHERE Match_Type=1 AND Match_CoverDate>'".date('Y-m-d H:i:s')."' AND Match_Date='".date("m-d")."' and Match_HalfId is not null ".$sqlwhere.' order by Match_CoverDate,iPage,match_name,Match_Master,Match_ID,iSn';
