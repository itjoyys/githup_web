...
$style = 'tj_ssc';
if (! empty($_GET['type'])) {
    $cq_ssc_odds = M("c_odds_10", $db_config)->where("type='" . $_GET['type'] . "'")->find();
...
//查当前玩法已下注的总额
$beted = beted_limit(10,$title_3d,$db_config);
$ball_limit_num = $beted['sum(money)'];
// 查询期数,中奖号
$qishu = M('c_auto_10', $db_config);
$num = $qishu->field("*")
    ->order("datetime desc")
    ->find();

//获取开盘时间，封盘时间开盘状态判断时间，封盘状态判断时间的数组
$array = set_arraypan(10,$db_config);
?>
<!DOCTYPE html>
...
<script type="text/javascript">
	...
	setInterval(function(){lottery(10,5,<?=$num['qishu']?>);},30000);
...
<?php  include("./include/common_limit.php");  ?>
		...
		<table border="0" cellpadding="0" cellspacing="1" class="game_table">
			<tbody>
				<tr class="tbtitle">
					<td colspan="15" id="time_td"><font>当前期数：<font class="colorDate"><b> <?=dq_qishu(10,$publicdb)?></b></font>
				...
				<form autocomplete="off" name="orders" id="order_form" method="get"
					...
					<input type="hidden" name="type" value="10"> <input type="hidden"
						name="qishu"
						value="<?=dq_qishu(10,$publicdb)?>">
					<tr class="tbtitle2">
						<th colspan="15"><div
...
$result = M("c_auto_10", $db_config)->field("*")
    ->where($map_where)
    ->select();
$gailv_arr = array();
if (! empty($result)) {
...
<?php }} ?>
		...
		<iframe src="./show_zoushi.php?type=10" width="100%" height="150"
