...
function sessionBet($uid){
	...
	if($_SESSION["bets"]<10) {
		...
		echo "<div class=\"pollbox\" id =\"idcs\"> 
				  ...
				  <p style=\" text-align:center\"></p>
				  <p style=\"font-size:12px; text-align: center; padding:5px; line-height: 20px;\"><font style=\"color:red;text-align:center;\">您点击次数太快了..<br />为了保证网站数据安全..<br />请您稍等<span id='miao'>30</span>秒后再操作..</font></p></div>
...
function islogin_match($uid){
	if($uid){
		...
		session_destroy();
		echo "<script>window.location.href='../../sport/select.php';</script>";
		exit;
	}
}
?>