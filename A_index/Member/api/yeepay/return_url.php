...
$user=M('k_user_bank_in_record',$db_config)->where("order_num='$_REQUEST[r6_Order]'")->find();
if($user['make_sure']==1){
...
if($bRet){
	if($r1_Code=="1"){
		...
		if($r9_BType=="1"){
			...
			$order_num=$_REQUEST['r6_Order'];
			$b = M('k_user_bank_in_record',$db_config);
				$userMoney = M('k_user',$db_config)->field("money,username")->where("uid = '".$uid."'")->find();

				//会员余额
				$iMoney=$b->field('id,deposit_money,order_num,favourable_num,other_num,deposit_num,do_time')->where("order_num = '".$order_num."'")->find();//会员存入总金额
				if($r3_Amt!=$iMoney['deposit_num']){
