...
$user=M('k_user_bank_in_record',$db_config)->where("order_num='$_REQUEST[merOrderNum]'")->find();
if($user['make_sure']==1){
...
if($signValue==$signValue2){
	if($respCode=='0000'){
				$order_num=$_REQUEST['merOrderNum'];
		...
		echo "<script>alert('交易失败,错误代码XS004');window.close();</script>";exit;
	}
}else{
	echo "<script>alert('交易失败,错误代码XS005');window.close();</script>";exit;
}
?>