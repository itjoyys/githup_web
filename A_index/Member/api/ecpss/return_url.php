...
$user=M('k_user_bank_in_record',$db_config)->where("order_num='$_REQUEST[BillNo]'")->find();
if($user['make_sure']==1){
	...
	$md5sign = strtoupper(md5($md5src));
	 ...
	 if ($SignMD5info==$md5sign){
	 	if ($Succeed=="88") {
			...
			echo '<script>alert("入款失败,错误代码XS004");window.close();</script>';exit;
		}
	}else
        {
        	echo '<script>alert("入款失败,错误代码XS005");window.close();</script>';exit;
		//验签失败 业务结束
	}
