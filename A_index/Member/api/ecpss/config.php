...
if(!empty($_SESSION['payid'])){//支付前
	...
	if(!empty($result)){
		...
		//跳转地址
		$m_okurl             = 'http://'.$result['f_url'].'/ecpss_rep.php';

		//返回地址
		$ServerUrl           = 'http://'.$result['f_url'].'/ecpss_callback.php';

		//提交地址
		$form_url            = 'https://'.$result['pay_domain'];

	}
}elseif(!empty($_REQUEST['BillNo'])){//支付完成 返回
	include_once("../../../include/filter.php");
	include_once("../../../include/private_config.php");
	include_once("../../../lib/class/model.class.php");
	$user    = '';
	$result  = '';
	$user=M('k_user_bank_in_record',$db_config)->field('pay_id')->where("order_num='$_REQUEST[BillNo]'")->find();
