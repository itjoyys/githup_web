...
if($action=='getlist'){
    ...
    if($did) $where.=" and agent_id='".$did."' ";
    if($gid) {
        
        $agent = M('k_user_agent',$_DBC['private'])->where("site_id = '".SITEID."' and is_demo = 0")->select();
        $agent_id = Level::getChildsId($agent,$gid,'a_t');
    ...
    $table=M("c_bet",$_DBC['private']);
    $DataList=$table->where($where)->order('id desc')->limit($limit)->select();
