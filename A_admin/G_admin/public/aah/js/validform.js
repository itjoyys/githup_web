/*
	...
	$(".demoform").Validform({//$(".demoform")指明是哪一表单需要验证,名称需加在form表单上;
		...
		callback:function(data){
			//返回数据data是json格式，{"info":"demo info","status1":"y"}
			//info: 输出提示信息;
			//status1: 返回提交数据的状态,是否提交成功。如可以用"y"表示提交成功，"n"表示提交失败，在ajax_post.php文件返回数据里自定字符，主要用在callback函数里根据该值执行相应的回调操作;
...
(function($,win,undef){
	...
	var Validform=function(forms,settings,inited){
		...
		forms.each(function(n){
			...
			//防止表单按钮双击提交两次;
			this.status1="normal"; //normal | posting | posted;
	...
	Validform.util={
		...
		showmsg:function(msg,type,o,show){
			...
			*/
			...
			if(type==2 && o.obj){
				if(o.obj.parent().is("div")){
					o.obj.parent().parent().find(".Validform_checktip").text(msg);
					Validform.util.cssctl(o.obj.parent().parent().find(".Validform_checktip"),o.type);
				}
				else
				{
		...
		submitForm:function(settings,flg,ajaxPost,sync){
			...
			*/
			...
			//表单正在提交时点击提交按钮不做反应;
			if(curform[0].status1==="posting"){return false;}
			
			//要求只能提交一次时;
			if(settings.postonce && curform[0].status1==="posted"){return false;}
				...
				if(settings.ajaxPost || ajaxPost==="ajaxPost"){
					...
					curform[0].ajax=$.ajax({
						...
						success: function(data){
							...
							curform[0].status1="posted";
							curform[0].ajax=null;
						},
						error: function(data){
							var msg=data.status1Text==="abort" ? 
