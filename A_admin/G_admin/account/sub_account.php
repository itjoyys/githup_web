  ...
  case 'pa':
    //暂停
      ...
      $stateP = M('sys_admin',$db_config)
              ->where("uid = '".$_GET['uid']."'")
              ->update($dataP);
      admin::insert_log($_SESSION["adminid"], $_SESSION['login_name'], "暂停了代理后台管理员 " . $_GET['sname']);
  ...
  case 'us':
    //启用
     ...
     $stateP = M('sys_admin',$db_config)
              ->where("uid = '".$_GET['uid']."'")
              ->update($dataP);
     admin::insert_log($_SESSION["adminid"], $_SESSION['login_name'], "启用了代理后台管理员 " . $_GET['sname']);
     admin::make_offline($uid);
  ...
  case 'de':
    //删除
     ...
     $stateP = M('sys_admin',$db_config)
              ->where("uid = '".$_GET['uid']."'")
              ->update($dataP);
      admin::insert_log($_SESSION["adminid"], $_SESSION['login_name'], "删除了代理后台管理员 " . $_GET['sname']);
      admin::make_offline($uid);
...
?>
    <tr  class="m_cen">
      <!-- <td><a  title=""  href="">離線</a></td> -->
        <td><a  style="color:red"  title="登陸時間:<?=date('Y-m-d H:i:s',$rows['updatetime'])?> 登陸IP:<?=$rows['login_ip']?>"  href="sub_account.php?mem_status=<?=$rows["is_login"]>0?1:0?>&page=<?=!empty($_GET['page'])?$_GET['page']:1?>&page_num=<?=!empty($_GET['page_num'])?$_GET['page_num']:20?>&search_type=<?=!empty($_GET['search_type'])?$_GET['search_type']:0?>&search_name=<?=$_GET['search_name']?>"><?=$rows["is_login"]>0 ? "<span style=\"color:#FF00FF;\">在線</span>" : "<span style=\"color:#999999;\">離線</span>" ?></a></td>
      <td><?=$rows["about"]?></td>
 ...
 <a  href="sub_account.php?action=de&sname=<?=$rows['login_name']?>&uid=<?=$rows["uid"]?>"  onclick="return confirm('确定删除该子账号?');">刪除</a>
  ...
  <tr class="m_rig" style="display:;">
        <td height="70" align="center" colspan="7"><font color="#3B2D1B">暫無數據。</font></td>
      </tr>
  <?php }?>
  </tbody>
  </table>
