...
foreach ($spArr as $key => $val) {
     $spType[$val] = $Spgame->join("join k_user_agent_sport_set on k_user_agent_sport_set.type_id = sp_games_view.id")->where("sp_games_view.type = '".$val."' and k_user_agent_sport_set.aid = '".$_SESSION['agent_id']."'")->select();
  }
$typeC = 'sp';
?>
...
<select onchange="document.location=this.value" name="stype">
  <option value="sp_list.php" <?php select_ed($_GET['stype'],'sp_list.php');?>>體育</option>
  <option value="fc_list.php" <?php select_ed($_GET['stype'],'fc_list.php');?>>彩票</option>
