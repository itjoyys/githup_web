...
class Info_case extends MY_Controller {
	...
	public function case_index(){
        ...
        $config['base_url'] = '/site_info/index.php/site_info/info_case/case_index?cs='.$map['cstate'];
        ...
        foreach ($case as $k => $v) {
           ...
           $case[$k]['url'] = self::get_view($v['id'],$v['type']);
        }
        $this->add("case",$case);
        switch ($cstate) {
            case '1':
    ...
    //获取预览网址
   function get_view($cid,$type){
       $www = $this->Info_case_model->rfind('web_config','site_id',$_SESSION['site_id'],'conf_www');
