...
?>
  ...
  <tr>
    ...
    <td  align="left" bgcolor="#3C4D82" style="color:#FFF"><strong>重庆时时彩</strong></td>
  </tr>
  <tr>
...
<?php
$sql		=	"select id from c_auto_2 order by qishu desc";
$query		=	$mysqli->query($sql);
...
if($id){
	...
	while($rows = $query->fetch_array()){
		...
		$hm[]		= $rows['ball_5'];
		if($rows['ok']==1){
			$ok = '<a href="js_2.php?qi='.$rows['qishu'].'"><font color="#FF0000">已结算</font></a>';
		}else{
			$ok = '<a href="js_2.php?qi='.$rows['qishu'].'"><font color="#0000FF">未结算</font></a>';
		}
?>
