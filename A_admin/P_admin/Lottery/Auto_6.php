...
?>
  ...
  <tr>
    ...
    <td  align="left" bgcolor="#3C4D82" style="color:#FFF"><strong>排列三</strong></td>
  </tr>
  <tr>
...
<?php
$sql		=	"select id from c_auto_6 order by qishu desc";
$query		=	$mysqli->query($sql);
...
if($id){
	...
	while($rows = $query->fetch_array()){
		...
		$hm[]		= $rows['ball_3'];
		if($rows['ok']==1){
			$ok = '<a href="js_6.php?qi='.$rows['qishu'].'"><font color="#FF0000">已结算</font></a>';
		}else{
			$ok = '<a href="js_6.php?qi='.$rows['qishu'].'"><font color="#0000FF">未结算</font></a>';
		}
?>
