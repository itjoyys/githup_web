...
if($id>0 && !isset($_GET['action'])){
	$sql	=	"select * from c_auto_1 where id=$id limit 1";
...
?>
  ...
  <tr>
    ...
    <td  align="left" bgcolor="#3C4D82" style="color:#FFF"><strong>广东快乐十分</strong></td>
  </tr>
  <tr>
...
<?php
$sql		=	"select id from c_auto_1 order by qishu desc";
$query		=	$mysqli->query($sql);
...
if($id){
	$id	=	rtrim($id,',');
	$sql	=	"select * from c_auto_1 where id in($id) order by qishu desc";
	$query	=	$mysqli->query($sql);
	$time	=	time();
