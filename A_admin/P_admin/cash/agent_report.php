...
if (empty($_GET['agent_id']) || empty($_GET['date_start']) || empty($_GET['date_end'])) {
	exit('system error code :0000');
}
//时间
...
$_GET['g_type'] = array('mg','ag','og','ct','bbin','lebo','mgdz');
//占成比例
$scale = M('k_user_agent',$db_config)->field("video_scale,sports_scale,lottery_scale")
      ->where("id = '".$_GET['agent_id']."'")
      ->find();
//彩票
if (!empty($_GET['cp']) && $_GET['cp'] == '2') 
{	
	$mapFc['agent_id'] = $_GET['agent_id'];
	$mapFc['js'] = 1;
	$mapFc['addtime'] = array(
...
if (!empty($_GET['g_type'])) {
	...
	$videoBet = array();
	$date = array();
	$date[0] = $_GET["date_start"].' 00:00:00';
	$date[1] = $_GET["date_end"].' 23:59:59';

	foreach ($_GET['g_type'] as $key => $val) {
		 $types[$val]['type'] = $val;
		 if ($val == 'mgdz') {
              $vBet = $games->GetUserAvailableAmountByAgentid('mg',$_GET['agent_id'], $date[0], $date[1],1);
          }elseif($val == 'bbdz'){
              $vBet = $games->GetUserAvailableAmountByAgentid('bbin',$_GET['agent_id'], $date[0], $date[1],5);
          }else{
              $vBet = $games->GetUserAvailableAmountByAgentid($val,$_GET['agent_id'], $date[0], $date[1]);
          }
       ...
       <?php 
           $Tnum = $TbetAll = $Tbet= $Tdelivery= $Twin= $Tbw = 0;
			...
			<tr class="m_rig" align="left">
				...
				<td><?=$va['bet']?></td>
				<td><?=$va['win']?></td>
				<td><?=sprintf("%.2f",($va['bet']-$va['win']))?></td>
				<td>0.00</td>
				<td><?=sprintf("%.2f",($va['bet']-$va['win']))?></td>
		        <td><?=sprintf("%.2f",($va['bet']-$va['win']))?></td>
		        <td><?=$va['delivery']?></td>
