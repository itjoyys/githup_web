        ...
        <td width="130">
            <div  class="input_002"><?
                ...
                if($_GET['MacthType'] ){
                    ...
                    echo      $macth_name= $_GET['MacthType'].'-入球数';
                }
                else{
                    $macth_name_="足球$mach_type_name";
                    echo $macth_name="足球$mach_type_name-入球数";
                }
                ?></div>
        </td>
        <td width="250">
...
<table width="526" border="0" cellpadding="0" cellspacing="1"  bgcolor="006255" class="m_tab" id="glist_table">
    ...
    foreach($arr_m as $k=>$v){
        ...
        <tr bgcolor="<?=$color?>" onMouseOver="this.style.backgroundColor='#EBEBEB'" onMouseOut="this.style.backgroundColor='<?=$color?>'">
            ...
            <td align="right"><a href="list.php?match_id=<?=$rows["Match_ID"]?>&column_like=Match_Total" <?=getAC($arr[$rows["Match_ID"]]["match_total"]['num'])?> ><?=getString($arr[$rows["Match_ID"]]["match_total"]['num'])?>/<?=getString($arr[$rows["Match_ID"]]["match_total"]['money'])?></a></td>
        </tr>
    <? }?>
</table>
