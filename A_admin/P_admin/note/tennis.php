...
if($type=='danshi'){
	$match_name=array();
	$sqlname		="select Match_Name from tennis_Match WHERE Match_Type=1 and Match_Date='".date("m-d",strtotime($date))."' order by Match_CoverDate,Match_Name";
	...
	$sql		=	"select id from tennis_Match WHERE Match_Type=1 and Match_Date='".date("m-d",strtotime($date))."' ".$sqlwhere." order by Match_CoverDate,Match_Name";
	//print_r($sql);exit;
...
if($type=='kaisai'){
	$match_name=array();
	$sqlname		="select Match_Name from tennis_Match WHERE Match_Type=1 and Match_Date='".date("m-d",strtotime($date))."' and Match_CoverDate<'".date("Y-m-d H:i:s")."' order by Match_CoverDate,Match_Name";
	$query		=	$mysqli->query($sqlname);
	while($row = $query->fetch_array()){
		if(!in_array($row['match_name'],$match_name)){
			$match_name[]=$row['match_name'];
	...
	$sql		=	"select id from tennis_Match WHERE Match_Type=1 and Match_Date='".date("m-d",strtotime($date))."' and Match_CoverDate<'".date("Y-m-d H:i:s")."' ".$sqlwhere." order by Match_CoverDate,Match_Name";
	//print_r($sql);exit;
	$query		=	$mysqli->query($sql);
	//分页
	$sum    = $mysqli->affected_rows;//总页数
...
<div  class="input_002">網球-<?=$type_name?></div>
...
<div  id="dt_now">--美東時間:2014-11-29 04:36:31</div>
 &nbsp;&nbsp;
<a  href="tennis.php?type=danshi" <?if($_GET['type']=='danshi'){echo 'style="color:#F00"';}?>>單式</a>
<a  href="tennis.php?type=kaisai" <?if($_GET['type']=='kaisai'){echo 'style="color:#F00"';}?>>已开赛</a>
</div>
