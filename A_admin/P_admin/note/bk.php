...
if($type=='danshi'){
	...
	$match_name=array();
	$sqlname		="select match_name from lq_match WHERE Match_Type!=3 AND Match_Date='".date("m-d",strtotime($date))."'  order by Match_CoverDate,iPage,iSn,Match_ID,match_name,Match_Master";
	...
	$sql		=	"select id from lq_match WHERE Match_Type!=3 AND Match_Date='".date("m-d",strtotime($date))."' ".$sqlwhere.'  order by Match_CoverDate,iPage,iSn,Match_ID,match_name,Match_Master';
	//print_r($sql);exit;
...
if($type=='gunqiu'){
	$match_name=array();
	$sqlname		="select match_name from lq_match WHERE Match_Type!=3 and Match_IsLose='1' AND Match_Date='".date("m-d",strtotime($date))."' order by Match_CoverDate,iPage,iSn,Match_ID,match_name,Match_Master";
	$query		=	$mysqli->query($sqlname);
	while($row = $query->fetch_array()){
		if(!in_array($row['match_name'],$match_name)){
			$match_name[]=$row['match_name'];
	...
	$sql		=	"select id from lq_match WHERE Match_Type!=3 and Match_IsLose='1' AND Match_Date='".date("m-d",strtotime($date))."' ".$sqlwhere.'  order by Match_CoverDate,iPage,iSn,Match_ID,match_name,Match_Master';
	$query		=	$mysqli->query($sql);
	//分页
	$sum    = $mysqli->affected_rows;//总页数
	$pagenum=isset($_GET['page_num'])?$_GET['page_num']:20; //每页显示的记录数
...
if($type=='kaisai'){
	$match_name=array();
	$sqlname		="select match_name from lq_match WHERE Match_Type!=3 and Match_CoverDate<'".date("Y-m-d H:i:s")."' AND Match_Date='".date("m-d",strtotime($date))."'  order by Match_CoverDate,iPage,iSn,Match_ID,match_name,Match_Master";
	$query		=	$mysqli->query($sqlname);
	while($row = $query->fetch_array()){
		if(!in_array($row['match_name'],$match_name)){
			$match_name[]=$row['match_name'];
	...
	$sql		=	"select id from lq_match WHERE Match_Type!=3 and Match_CoverDate<'".date("Y-m-d H:i:s")."' AND Match_Date='".date("m-d",strtotime($date))."' ".$sqlwhere.'  order by Match_CoverDate,iPage,iSn,Match_ID,match_name,Match_Master';
	$query		=	$mysqli->query($sql);
	//分页
	$sum    = $mysqli->affected_rows;//总页数
	$pagenum=isset($_GET['page_num'])?$_GET['page_num']:20; //每页显示的记录数
...
<div  class="input_002">篮球-<?=$type_name?></div>
...
<div  id="dt_now">--美東時間:2014-11-29 04:36:31</div>
 &nbsp;&nbsp;
<a  href="bk.php?type=danshi" <?if($_GET['type']=='danshi'){echo 'style="color:#f00"';}?>>單式</a>
<a  href="bk.php?type=gunqiu" <?if($_GET['type']=='gunqiu'){echo 'style="color:#f00"';}?>>滾球</a>
<a  href="bk.php?type=kaisai" <?if($_GET['type']=='kaisai'){echo 'style="color:#f00"';}?>>已开赛</a>
</div>
	...
	<?for($i=0; $i<count($json['db']); $i++){?>	
		...
		<td>
			<table  width="100%"  border="0"  cellspacing="0"  cellpadding="0">
				<tbody>
					<tr align="right">
					  <td align="right" width="33%"><?if($json['db'][$i]["Match_DsDpl"]>"0"){echo '单';}?>&nbsp;&nbsp;&nbsp;<?if($json['db'][$i]["Match_DsDpl"]>"0"){echo $json['db'][$i]["Match_DsDpl"];}?></td>
					  <td width="67%"><a href="#" style="color:#CF2878">0/0</a></td>
					</tr>
					<tr align="right">
					  <td align="right" width="33%"><?if($json['db'][$i]["Match_DsSpl"]!="0"){echo '双';}?>&nbsp;&nbsp;&nbsp;<?if($json['db'][$i]["Match_DsDpl"]<="0"){echo $json['db'][$i]["Match_DsDpl"];}?></td>
					  <td><a href="#" style="color:#CF2878">0/0</a></td>
					</tr>
