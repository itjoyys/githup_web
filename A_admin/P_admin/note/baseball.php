...
if($type=='danshi'){
	$match_name=array();
	$sqlname		=	"select match_name from baseball_match WHERE Match_Type=1 and Match_Date='".date("m-d",strtotime($date))."' order by Match_CoverDate,match_name";
	...
	$sql		=	"select id from baseball_match WHERE Match_Type=1 and Match_Date='".date("m-d",strtotime($date))."' ".$sqlwhere.' order by Match_CoverDate,match_name';
	//print_r($sql);exit;
...
if($type=='kaisai'){
	$match_name=array();
	$sqlname		=	"select match_name from baseball_match WHERE Match_Type=1 and Match_Date='".date("m-d",strtotime($date))."' and Match_CoverDate<'".date("Y-m-d H:i:s")."' order by Match_CoverDate,match_name";
	$query		=	$mysqli->query($sqlname);
	while($row = $query->fetch_array()){
		if(!in_array($row['match_name'],$match_name)){
			$match_name[]=$row['match_name'];
	...
	$sql		=	"select id from baseball_match WHERE Match_Type=1 and Match_Date='".date("m-d",strtotime($date))."' and Match_CoverDate<'".date("Y-m-d H:i:s")."' ".$sqlwhere.' order by Match_CoverDate,match_name';
	//print_r($sql);exit;
	$query		=	$mysqli->query($sql);
	//分页
	$sum    = $mysqli->affected_rows;//总页数
...
<div  class="input_002">棒球-<?=$type_name?></div>
...
<div  id="dt_now">--美東時間:2014-11-29 04:36:31</div>
 &nbsp;&nbsp;
<a  href="baseball.php?type=danshi" <?if($_GET['type']=='danshi'){echo 'style="color:#f00;"';}?>>單式</a>
<a  href="baseball.php?type=kaisai" <?if($_GET['type']=='kaisai'){echo 'style="color:#f00;"';}?>>已开赛</a>
</div>
	...
	<?for($i=0; $i<count($json['db']); $i++){?>	
		...
		<td>
			...
			<tbody>
				<tr  align="right">
					<td  width="52%">
						<?if($json['db'][$i]["Match_ShowType"]=="H"&&$json['db'][$i]["Match_Ho"]!="0"){echo $json['db'][$i]["Match_RGG"];}?>&nbsp;&nbsp;&nbsp;<?if($json['db'][$i]["Match_Ho"]<="0"){echo $json['db'][$i]["Match_Ho"];}?>
					</td>
					<td><a href="#" style="color:#CF2878">0/0</a></td>
				</tr>
				<tr  align="right">
					<td>
						<?if($json['db'][$i]["Match_ShowType"]=="C"&&$json['db'][$i]["Match_Ao"]!="0"){echo $json['db'][$i]["Match_RGG"];}?>&nbsp;&nbsp;&nbsp;<?if($json['db'][$i]["Match_Ao"]<="0"){echo $json['db'][$i]["Match_Ao"];}?>
		...
		<td>
			<table  width="100%"  border="0"  cellspacing="0"  cellpadding="0">
				<tbody>
					<tr  align="right">
						<td  width="52%">
							<?if($json['db'][$i]["Match_DxGG"]>"0" && $json['db'][$i]["Match_DxGG"]!=""){echo 'O'.$json['db'][$i]["Match_DxGG"];}?>&nbsp;&nbsp;&nbsp;
							<? if($json['db'][$i]["Match_DxDpl"]<='0'){echo '';}else{echo $json['db'][$i]["Match_DxDpl"];}?>
						</td>
						<td><a href="#" style="color:#CF2878">0/0</a></td>
					</tr>
					<tr  align="right">
						<td>
							<?if($json['db'][$i]["Match_DxGG"]>"0"){echo 'U'.$json['db'][$i]["Match_DxGG2"];}?>&nbsp;&nbsp;&nbsp;
							<? if($json['db'][$i]["Match_DxXpl"]<="0"){echo $json['db'][$i]["Match_DxXpl"];}?>
						</td>
						<td><a href="#" style="color:#CF2878">0/0</a></td>
					</tr>
					<tr>
