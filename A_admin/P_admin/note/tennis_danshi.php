            ...
            <tr bgcolor="<?=$color?>" onMouseOver="this.style.backgroundColor='#EBEBEB'" onMouseOut="this.style.backgroundColor='<?=$color?>'">
                ...
                <td><table cellspacing="0" cellpadding="0" width="100%" border="0">
                        ...
                        <tr align="right">
                            ...
                            <td width="67%">
                                <? if($rows["Match_BzM"]>0){?>
                                    <a href="list.php?match_id=<?=$rows["Match_ID"]?>&point_column=Match_BzM" <?=getAC($arr[$rows["Match_ID"]]["match_bzm"]['num'])?> ><?=getString($arr[$rows["Match_ID"]]["match_bzm"]['num'])?>/<?=getString($arr[$rows["Match_ID"]]["match_bzm"]['money'])?></a><?}?></td>
                        </tr>
                        <tr align="right">
                            <td align="right"><? if($rows["Match_BzG"]>0) echo double_format($rows["Match_BzG"]);?>                <br /></td>
                            <td>
                                <? if($rows["Match_BzM"]>0){?>
                                    <a href="list.php?match_id=<?=$rows["Match_ID"]?>&point_column=Match_BzG" <?=getAC($arr[$rows["Match_ID"]]["match_bzg"]['num'])?> ><?=getString($arr[$rows["Match_ID"]]["match_bzg"]['num'])?>/<?=getString($arr[$rows["Match_ID"]]["match_bzg"]['money'])?></a><?}?></td>
                        </tr>
                        </tbody>
                    </table></td>
            </tr>
