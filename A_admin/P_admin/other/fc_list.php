...
foreach ($fcArr as $key => $val) {
   $fcType[$val] = $Fcgame->join("join k_user_agent_fc_set on k_user_agent_fc_set.type_id = fc_games_view.id")->where("fc_games_view.fc_type = '".$val."' and k_user_agent_fc_set.is_default = '1' and k_user_agent_fc_set.site_id ='".SITEID."'")->select();
}
