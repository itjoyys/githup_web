...
$map_cp = " history_login.site_id = '".SITEID."' ";
//时间检索
...
$map_cp .= " and history_login.login_time >= '".$tmp_sdate."' and history_login.login_time <= '".$tmp_edate."' ";


//账号检索
if(!empty($_GET['username'])){
	$user='%'.$_GET['username'].'%';
	$map_cp .= " and history_login.username like '".$user."' ";
}
//ip检索
if(!empty($_GET['ip'])){
	$ip_str = '%'.$_GET['ip'].'%';
  $map_cp .= " and history_login.ip like '".$ip_str."' ";
}
//登录状态
if(!empty($_GET['state'])){
   if ($_GET['state'] == '2') {
      $map_cp .= " and history_login.state = '0' ";
   }else{
      $map_cp .= " and history_login.state = '1' ";
   }
}

$u = M('history_login',$db_config);
//总数
...
$user_login=$u->join("left join k_user_level on k_user_level.id = history_login.level_id")->field("history_login.*,k_user_level.level_des")->where($map_cp)->order("history_login.id DESC")
          ->limit($limit)-> select();

$page = $u->showPage($totalPage,$page);

?>
<?php $title="登陆日志"; require("../common_html/header.php");?>
<body>
<script>
    //分页跳转
...
// </script>
<div id="con_wrap">
  <div class="input_002">会员登錄日志</div>
  <div class="con_menu">
    <form method="get" name="action_form" id="myform">
      <a href="./member_login_log.php" style="color: rgb(255, 0, 0);">会员登陆</a>
      <a href="./sys_login_log.php">管理登陆</a>
      日期：
