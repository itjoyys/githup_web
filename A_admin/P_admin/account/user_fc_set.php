...
if ($_POST['type'] == 'setOne' && !empty($_POST['id'])) {
   ...
   unset($mapA['aid']);
   $mapA['uid'] = $_POST['uid'];
   $uFCstate = $Uaset->where($mapA)->find();
   ...
   if ($spSta) {
       $do_log = '成功编辑会员彩票设定:'.$_POST['type_id'];
       admin::insert_log($_SESSION['adminid'],$_SESSION['login_name'],$do_log);
       message("修改成功");
   }else{
      $do_log = '失败编辑会员彩票设定:'.$_POST['type_id'];
      admin::insert_log($_SESSION['adminid'],$_SESSION['login_name'],$do_log);
      message("修改失败");
   }
}else{
   foreach ($fcArr as $key => $val) {
     $tmpA = $tmpB = array();
     $tmpA = $Fcgame->join("join k_user_agent_fc_set on k_user_agent_fc_set.type_id = fc_games_view.id")->where("fc_games_view.fc_type = '".$val."' and k_user_agent_fc_set.aid = '".$_GET['aid']."'")->select("type");
     $tmpB= $Fcgame->join("join k_user_fc_set on k_user_fc_set.type_id = fc_games_view.id")->where("fc_games_view.fc_type = '".$val."' and k_user_fc_set.uid = '".$_GET['uid']."'")->select("type");
...
<?php foreach ($fcType as $key => $fcval): ?>
  ...
  <tr>
   ...
   <td nowrap="" align="center"><a href=""></a></td>
       <?php foreach ($fcval  as $k => $v): ?>
      <td><a href="javascript:void(0)" onclick="show_win('<?=$fcTitle[$key]?>-<?=$v['name']?>','<?=$v['charges_a']?>','<?=$v['single_field_max']?>','<?=$v['single_note_max']?>','<?=$v['min']?>','<?=$v['id']?>','<?=$v['type_id']?>','<?=$v['uid']?>');"> 修改</a></td>
       <?php endforeach ?>
  <tr>
</tbody></table>
