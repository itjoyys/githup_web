...
if ($_POST['type'] == 'setOne' && !empty($_POST['id'])) {
   ...
   unset($mapA['aid']);
   $mapA['uid'] = $_POST['uid'];
   $uSPstate = $Uaset->where($mapA)->find();
   ...
   if (empty($aid) && empty($uSPstate)) {
      //为空表示添加
      $_POST['site_id'] = SITEID;
      $spSta = $Uaset->add($_POST);
   }else{
      //更新
      unset($_POST['uid']);
      $spSta = $Uaset->where("id = '".$idS."'")->update($_POST);
   }

   if ($spSta) {
       $do_log = '成功编辑会员体育设定:'.$_POST['type_id'];
       admin::insert_log($_SESSION['adminid'],$_SESSION['login_name'],$do_log);
       message("修改成功");
   }else{
      $do_log = '失败编辑会员体育设定:'.$_POST['type_id'];
      admin::insert_log($_SESSION['adminid'],$_SESSION['login_name'],$do_log);
      message("修改失败");
   }
}else{
   foreach ($spArr as $key => $val) {
     $tmpA = $tmpB = array();
     $tmpA = $Spgame->join("join k_user_agent_sport_set on k_user_agent_sport_set.type_id = sp_games_view.id")->where("sp_games_view.type = '".$val."' and k_user_agent_sport_set.aid = '".$_GET['aid']."'")->select("t_type");
     $tmpB= $Spgame->join("join k_user_sport_set on k_user_sport_set.type_id = sp_games_view.id")->where("sp_games_view.type = '".$val."' and k_user_sport_set.uid = '".$_GET['uid']."'")->select("t_type");
...
設定類型：
  ...
  <option value="user_set.php?aid=<?=$_GET['aid']?>&uid=<?=$_GET["uid"]?>" <?php select_ed($typeC,'sp');?>>體育</option>
...
<table width="780" border="0" cellspacing="0" cellpadding="0" class="m_tab">
  ...
  <tr>
   ...
   <td nowrap="" align="center"><a href=""></a></td>
       <?php foreach ($spval as $k => $v): ?>
      <td><a href="javascript:void(0)" onclick="show_win('<?=$spTitle[$key]?>-<?=$v['t_name']?>','<?=$v['water_break']?>','<?=$v['single_field_max']?>','<?=$v['single_note_max']?>','<?=$v['min']?>','<?=$v['id']?>','<?=$v['type_id']?>','<?=$v['uid']?>');"> 修改</a></td>
    <?php endforeach ?>
  <tr>
</tbody></table>
<?php endforeach ?>
...
<div id="rs_window">
    ...
    <input type="hidden" id="uid" name="uid" value="<?=$_GET['uid']?>">
     <input type="hidden" id="agent_id" name="agent_id" value="<?=$_GET['aid']?>">
