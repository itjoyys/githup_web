...
if(!empty($_GET['account'])){
    ...
    $str_username = '%'.$_GET['account'].'%';
		$sql ="site_id = '".SITEID."' and username like '".$str_username."' and shiwan <> 1 "; 
  ...
  //股东
    $str_at = '%'.$_GET['account'].'%';
    $sql ="k_user_agent.site_id='".SITEID."' and k_user_agent.agent_user like '".$str_at."' and k_user_agent.agent_type = 's_h'  and k_user_agent.is_demo = '0'";
    $user=M('k_user_agent',$db_config)->field("id,pid,agent_user")->where($sql)->select();
     $agent_1=M('k_user_agent',$db_config)->where("agent_type <> 's_h'")->select();
     if(!$user){
      message('没有此股东!','account_search.php');
    }
    foreach ($user as $key => $val) {

         $c_data = Level::getChildsCount($agent_1,$val['id'],'u_a','a_t');
         $user[$key]['s_h'] = $val['agent_user'];
         $user[$key]['u_a'] = $c_data['u_a']+0;
         $user[$key]['a_t'] = $c_data['a_t']+0;

         $agents_id = implode(',',Level::getChildsId($agent_1,$val['id'],'a_t'));//股东旗下所有会员id
         if (!empty($agents_id)) {
            $map = "agent_id in ($agents_id) and site_id ='".SITEID."' and shiwan <> '2'";
            $user[$key]['username'] = M('k_user',$db_config)->field('uid')->where($map)->count();
         }else{
            $user[$key]['username'] = 0;
         }
    } 
...
<select  name="search_type" id="search_type">
  <option value="4" <?php select_check($_GET['search_type'],4)?>>会员</option>
	<option value="1" <?php select_check($_GET['search_type'],1)?>>股东</option>
	<option value="2" <?php select_check($_GET['search_type'],2)?>>总代理</option>
	<option value="3" <?php select_check($_GET['search_type'],3)?>>代理</option>
  <option value="5" <?php select_check($_GET['search_type'],5)?>>推广ID</option>
</select>
