...
{
    var DG = new J.dialog({title:"存入 - "+user,iconTitle:false,maxBtn:false,btnBar:false,drag:true,cover:true,bgcolor:'#3B2D1B',autoSize:true,id:'ch_quick',page:"../pub/user_cz.php?u="+user+"&t="+t, width:310, height:90,
    dgOnLoad: function(){ 
...
{
    var DG = new J.dialog({title:"提取 - "+user,iconTitle:false,maxBtn:false,btnBar:false,drag:true,cover:true,bgcolor:'#3B2D1B',autoSize:true,id:'ch_quick',page:"../pub/user_tq.php?u="+user+"&t="+t, width:310, height:90,
    dgOnLoad: function(){ 
            DG.iDoc('ch_quick').getElementById('money').focus();
        }
    }); 
...
{
    var DG = new J.dialog({title:"現金额度查询 ["+user+"]",iconTitle:false,btnBar:false,drag:true,bgcolor:'#3B2D1B',cover:true,id:'fd_quick',page:"../pub/user_search.php?u="+user+"&t="+t, width:500,height:window.screen.height-280,lockScroll:false}); 
    DG.ShowDialog();

}
function OpenDetail(qs,lx,id,n,lx2)
{
    art.dialog.open("/app/ssc/admin/main.php?uid="+uid+"&langx="+langx+"&action=look&lx2="+lx2+"&act=main&kithe="+qs+"&lx="+lx+"&id="+id+"&class3="+n+(typeof($('#tm').val())!="undefined"?"&tm="+$('#tm').val():"")+"&zc="+$("#zc").val()+"&ab="+$("#ab").val(),{
	  title: qs+"期-明細",
...
{
		/*var DG = new J.dialog({title:"走飞",iconTitle:false,maxBtn:false,btnBar:false,drag:true,bgcolor:'#3B2D1B',cover:true,id:'fd_quick',page:"main.php?action=d1&class1="+class1+"&class2="+class2+"&class3="+class3+"&rate="+rate+"&ts="+ts+"&money="+money, width:228,height:195,lockScroll:false}); 
    DG.ShowDialog();
    return false;*/
}
function CanceWin()
...
function OpenGg()
{
	var DG = new J.dialog({title:"最新公告",iconTitle:false,btnBar:false,drag:true,bgcolor:'#3B2D1B',cover:true,id:'fd_quick',page:"/gonggao.php", width:600,height:400,lockScroll:false}); 
	DG.ShowDialog();
}
/////表择表单////_b为图片名///////////////
