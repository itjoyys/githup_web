 ...
 *	version 0.3.6
 * Author: Jim Palmer
 * Released under MIT license.(It's a little changed by yy :P)
 */
(function($) {
	...
	$.jCal = function (target, opt) {
		opt = $.extend({
			...
			changeCal:		function (day, direct) { return true;},				// this key-value pair is defined by yy own. please read refered in line 155 and line 197.
			...
			$(target).find('.jCal .monthName, .jCal .monthYear')
				...
				.bind('click', $.extend( {}, opt ),
					function (e) {
						...
						//.....
						$(moSel)
							.css('clip','rect(' + diffOff + ' ' + ( $(this).width() + ( pad * 2 ) ) + ' '+ diffOff + ' 0)')
...
//以下是原来的代码，但会在ie下选择月份和年份时会发生jquery内部脚本错误，所以经过更改后保留在上面，而注释原来的代码
//						$(moSel)
//						.css('clip','rect(' + diffOff + ' ' + ( $(this).width() + ( pad * 2 ) ) + ' '+ diffOff + ' 0)')
//						.animate({opacity:0.92,'clip':'rect(0 ' + ( $(this).width() + ( pad * 2 ) ) + ' ' + $(moSel).height() + ' 0)'}, 'slow', function () {
//								$(this).parent().find('.monthSelectorShadow').bind('mouseover click', function () { $(this).parent().find('.monthSelector').remove(); $(this).remove(); });
//							})
//						.parent().find('.monthSelectorShadow').animate({'opacity':.1}, 'fast');
						$('.jCalMo .monthSelect', e.data._target).bind('mouseover mouseout click', $.extend( {}, e.data ), 
							function (e) {
								if ( e.type == 'click' ){
									//增加以下判断，解决月份列表选择时方格状态不显示的问题
									if(typeof(opt.changeCal) == "function")
									{
										setTimeout(function(){opt.changeCal(e.data.day, 'null');}, 50); //because above code not run finished, so until balabala...must be slowly.
									}//*
		...
		$(target).find('.jCal .left').bind('click', $.extend( {}, opt ),
			function (e) {
				$.ajax({
					...
					success:function(){
						...
						$('.jCalMo', e.data._target).wrapAll(
							'<div class="jCalMask" style="clip:rect(0 '+mD.w+' '+mD.h+' 0); width:'+ ( mD.w + ( mD.w / e.data.showMonths ) ) +'px; height:'+mD.h+'px;">' + 
				...
				if(typeof(opt.changeCal) == "function")
				{
					setTimeout(function(){opt.changeCal(e.data.day, "left");}, 700); //because above code not run finished, so until balabala...must be slowly.
				}				
			});
		$(target).find('.jCal .right').bind('click', $.extend( {}, opt ),
			function (e) {
				...
				if(typeof(opt.changeCal) == "function")
				{
					setTimeout(function(){opt.changeCal(e.data.day, 'right');}, 50);	//the code style would be same above code, so ... but can be quickly :p
				}
			});
		$('.jCal', target).each(
			function () {
