...
function annulCard($p2_Order,$p3_Amt,$p4_verifyAmt,$p5_Pid,$p6_Pcat,$p7_Pdesc,$p8_Url,$pa_MP,$pa7_cardAmt,$pa8_cardNo,$pa9_cardPwd,$pd_FrpId,$pz_userId,$pz1_userRegTime,$p1_MerId,$merchantKey,$reqURL_SNDApro){
  ...
  $unkonw				= "";							#未知错误
	...
	if($sNewString==$hmac) {
		if($r1_Code=="1"){
				return "<br>提交成功!".$rq_ReturnMsg."<br>商户订单号:".$r6_Order."<br>";
		} else if($r1_Code=="2"){
		      return "<br>提交失败".$rq_ReturnMsg."<br>支付卡密无效!";
		} else if($r1_Code=="7"){
		      return "<br>提交失败".$rq_ReturnMsg."<br>支付卡密无效!";
		} else if($r1_Code=="11"){
		      return "<br>提交失败".$rq_ReturnMsg."<br>订单号重复!";
		} else{
		      return "<br>提交失败".$rq_ReturnMsg."<br>请检查后重新支付";
		}
	} else{
		return "<br>localhost:".$sNewString."<br>YeePay:".$hmac."<br>交易签名无效!";
	}
}
...
function arrToStringDefault($arr)
{
	...
	function get_all_info($url,$order_num,$s_amount,$bank,$pay_id,$pay_key,$username,$pa7_cardAmt,$pa8_cardNo,$pa9_cardPwd){
			$p1_MerId = $pay_id;//商户id
			$merchantKey = $pay_key;//商户密钥
			$ServerUrl = 'http://'.$url.'/index.php/pay/card_callback';//返回地址
			$reqURL_SNDApro = 'https://www.yeepay.com/app-merchant-proxy/command.action';
			//提交地址
			...
			$pz_userId = $username;#用户唯一标识
			$pz1_userRegTime = date('Y-m-d H:i:s');#用户的注册时间
			return $this->annulCard($p2_Order,$p3_Amt,$p4_verifyAmt,$p5_Pid,$p6_Pcat,$p7_Pdesc,$p8_Url,$pa_MP,$pa7_cardAmt,$pa8_cardNo,$pa9_cardPwd,$pd_FrpId,$pz_userId,$pz1_userRegTime,$p1_MerId,$merchantKey,$reqURL_SNDApro);
	}
}