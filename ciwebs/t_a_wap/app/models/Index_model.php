...
class Index_model extends MY_Model {
	...
	//游戏问题
	//获取站点基本配置信息
	public function GetSiteBasicConfig($sid,$indexid){
        ...
        $data['ErrorCode'] = 0;
        $data['Data']['SiteNo'] = $sid."_".$indexid;
        $data['Data']['RegStep'] = empty($result["is_wap_one"])?2:1;
        $data['Data']['GameClassList'][0] = array('GameClassID'=>1,'GameClassName'=>'彩票游戏');
        $data['Data']['GameClassList'][1] = array('GameClassID'=>2,'GameClassName'=>'体育游戏');
        $data['Data']['GameClassList'][2] = array('GameClassID'=>3,'GameClassName'=>'AG视讯');
        $data['Data']['GameClassList'][3] = array('GameClassID'=>4,'GameClassName'=>'LEBO视讯');
        $data['Data']['GameClassList'][4] = array('GameClassID'=>5,'GameClassName'=>'BBIN视讯');
        $data['Data']['GameClassList'][5] = array('GameClassID'=>8,'GameClassName'=>'MG电子');
        $data['Data']['PromoCode'] = '';
        $data['Data']['SiteName'] = 'zh_cn:'.$copyright['copy_right'].',zh_tw:'.$copyright['copy_right'];
        $data['Data']['Lang'] = 'zh_cn,zh_tw';
        $data['Data']['Theme'] = 'default';
        $data['Data']['LogoUrl'] = $this->GetWapLogo(SITEID,INDEX_ID);//logo地址
        $data['Data']['DesktopIconUrl'] = '/public/cdn/16247443.png';//ico图标
        $data['Data']['WebName'] = $copyright['web_name'];
        $data['Data']['Template'] = 'default';
        $data['Data']['Intr'] = empty($_SESSION["intr"])?"":$_SESSION["intr"];
        $data['Data']['ReturnMode'] = '1';
        $data['Data']['OnLineServiceLink'] = str_replace("&amp;", "&", $copyright['online_service']);
        $data['Data']['TopbarBgColor'] = !empty($copyright['topbarbgcolor']) ? trim($copyright['topbarbgcolor']) : "#feb53b";
        $data['Data']['TryPlaySet'] = true;

        return $data;
	}

	//获取logo
	public function GetWapLogo($site_id,$index_id){
		$this->db->from('info_logo_use');
		$this->db->where('site_id',$site_id);
		$this->db->where('index_id',$index_id);
		$this->db->where('type',30);
		$this->db->where('state',1);
		$logo_info = $this->db->get()->row_array();
		if(!empty($logo_info['logo_url'])){
			return $logo_info['logo_url'];
		}else{
			$logourl = '/public/cdn/16211325.png';
			return $logourl;
		}
	}

	//获取轮播
	public function GetWapFlash($site_id,$index_id){
		$this->db->from('info_flash_use');
		$this->db->where('site_id',$site_id);
		$this->db->where('index_id',$index_id);
		$this->db->where('type',31);
		$flash_info = $this->db->get()->row_array();
		$result = array();
		if(!empty($flash_info)){
			$arr = array('A','B','C','D','E');
			foreach ($arr as $k => $v) {
				 ...
				 $url_field = 'url_'.$v;
	    	 	if(!empty($flash_info[$img_field]) && $flash_info[$img_field] != 1){
	    	 		$result[$v]['ActivityPlayImage'] = $flash_info[$img_field];
	    	 		$result[$v]['title'] = $flash_info[$title_field];
	    	 		$result[$v]['url'] = $flash_info[$url_field];
	    	 	}
			}
		}else{
	    	$result[0] = array('ActivityPlayImage'=>'/public/cdn/01.jpg');
			$result[1] = array('ActivityPlayImage'=>'/public/cdn/02.jpg');
			$result[2] = array('ActivityPlayImage'=>'/public/cdn/03.jpg');
		}
		return $result;
	}
	...
	//获取滚动图片
	public function GetPlayImages($sid,$indexid) {
	    $data['ErrorCode'] = 0;
	    $data['ErrorMsg'] = '执行成功';
		$data['Data']['PlayImageList'] = array();
		$data['Data']['PlayImageList'] = $this->GetWapFlash(SITEID,INDEX_ID);
		return $data;
	}
    //站点游戏类别
	public function GetGameState($sid,$indexid){
		//TODO 维护的状态
		//getweihu
        $data['ErrorCode'] = 0;
	    $data['ErrorMsg'] = '执行成功';
		...
		$sp = $this->maintenance_model->getweihu('sport')?0:1;
		$ag = $this->maintenance_model->getweihu('ag')?0:1;
		$lebo = $this->maintenance_model->getweihu('lebo')?0:1;
		$mgdz = $this->maintenance_model->getweihu('mg')?0:1;
		$bbin = 0;
		...
		$this->load->model('Common_model');
      	...
      	$data['Data'][0] = array('GameClassID'=>1,'State'=>$lot,'CloseTime'=>'',
	    	'OpenTime'=>'','GameClassName'=>'彩票游戏');
	    $data['Data'][1] = array('GameClassID'=>2,'State'=>$sp,'CloseTime'=>'',
	    	'OpenTime'=>'','GameClassName'=>'体育游戏');

      	if(in_array("ag", $video_modules)){
			 $data['Data'][] = array('GameClassID'=>3,'State'=>$ag,'CloseTime'=>'',
	    	'OpenTime'=>'','GameClassName'=>'AG视讯');
		}
		if(in_array("lebo", $video_modules)){
			$data['Data'][] = array('GameClassID'=>4,'State'=>$lebo,'CloseTime'=>'',
	    	'OpenTime'=>'','GameClassName'=>'LEBO视讯');
		}
		if(in_array("bbin", $video_modules)){
			$data['Data'][] = array('GameClassID'=>5,'State'=>$bbin,'CloseTime'=>'',
	    	'OpenTime'=>'','GameClassName'=>'BBIN视讯');
		}

		if(in_array("mg", $video_modules)){
			$data['Data'][] = array('GameClassID'=>8,'State'=>$mgdz,'CloseTime'=>'',
	    	'OpenTime'=>'','GameClassName'=>'MG电子');
		}
        return $data;
	}
    //首页滚动公告
	public function GetNewNotices($sid,$indexid){
		$v_c = $this->get_video_config();

		$this->db->from('site_notice');
		$map = "sid = '0' and notice_state = '1' and notice_cate < '6' and (notice_type in (".$v_c.") or notice_type is null)";
		$notice = $noticem = array();

		$notice = $this->db->select('id as NoticeID,notice_date as StartDateTime,notice_content as Content')->where($map)->order_by('notice_date DESC')->get()->result_array();

		$mapm = "site_id = '".SITEID."' and index_id = '".INDEX_ID."' and is_delete = 0 and show_type = 2 and state = 1";
		$noticem = $this->db->select('id as NoticeID,add_time as StartDateTime,chn_simplified as Content')->where($mapm)->order_by("add_time DESC")->get('k_message',8,0)->result_array();

		$notice = array_merge($notice, $noticem);

		$data['ErrorCode'] = 0;
		$data['ErrorMsg'] = '执行成功';
		$data['Data'] = array();
		foreach ($notice as $key => $val) {
		    $data['Data'][$key]['NoticeID'] = '"--'.$val['NoticeID'].'--"';
		    $data['Data'][$key]['StartDateTime'] = $val['StartDateTime'];
