...
class Games {
	...
	//登录mg电子
	function forwarddz($gtype,$loginname, $gameid, $lang = "CN", $cur = 'RMB',$gametype = '') {
		$crypt = $this->DES1($this->DESKey);
		$params = $this->encrypt("siteid=" . $this->siteid . "/\\\\/username=" . $loginname . "/\\\\/gametype=" . $gametype ."/\\\\/cur=" . $cur . "/\\\\/gameid=" . $gameid . "/\\\\/lang=" . $lang);
		$Key = $this->getKey($params);
		//echo "siteid=" . $this->siteid . "/\\\\/username=" . $loginname . "/\\\\/gametype=" . $gametype ."/\\\\/cur=" . $cur . "/\\\\/gameid=" . $gameid . "/\\\\/lang=" . $lang;
		//exit();
	...
	//$index_id
	function CreateAccount($loginname, $agent_id, $gtype, $index_id="a", $cur = 'RMB') {
		$crypt = $this->DES1($this->DESKey);
		if($gtype == 'pt'){
			$cur = 'CNY';
		}
		$params = $this->encrypt("siteid=" . $this->siteid . "/\\\\/username=" . $loginname . "/\\\\/agent_id=" . $agent_id ."/\\\\/index_id=" . $index_id . "/\\\\/cur=" . $cur);
		$Key = $this->getKey($params);
