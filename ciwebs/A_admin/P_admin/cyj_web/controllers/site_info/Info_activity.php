...
class Info_activity extends MY_Controller {
	...
	public function activity_index() {
		...
		$activity = $this->Info_activity_model->get_activity($pid,$index_id);
		foreach ($activity as $k => $val) {
			...
			$activity[$k]['height'] = $val['activity_h'];
                ...
                if (!empty($_SESSION['index_id'])) {
                    $this->add('sites_str','站点：'.str_replace('全部', '请选择站点',$this->Info_activity_model->select_sites()));
	...
	public function activity_edit()
    {
        ...
        $map = array();
        $map['table'] = 'info_activity_edit';
        $map['where']['id'] = $id;
        $map['where']['site_id'] = $_SESSION['site_id'];
        $activity = $this->Info_activity_model->rfind($map);
        $this->add("activity",$activity);
    ...
    public function activity_del(){
    	...
    	$where = array();
        $where['table'] = 'info_activity_edit';
        $where['where']['pid'] = $id;
        $where['where']['site_id'] = $_SESSION['site_id'];
        $activity = $this->Info_activity_model->rget($where);
        if($activity){
        	foreach ($activity as $key => $value) {
        		$ma = array();
        		$arr['state'] = 0;
		    	$ma = array();
		    	$ma['table'] = 'info_activity_edit';
		    	$ma['where']['id'] = $value['id'];
		    	$ma['where']['site_id'] = $_SESSION['site_id'];
		    	$this->Info_activity_model->rupdate($ma,$arr);
        	}
        }
    	$arr['state'] = 0;
    	$map = array();
    	...
    	$map['where']['id'] = $id;
    	$map['where']['site_id'] = $_SESSION['site_id'];
    	if ($this->Info_activity_model->rupdate($map,$arr)) {
	...
	public function activity_case(){
                $index_id = $this->input->get('index_id');
                $index_id = empty($index_id)?'a':$index_id;
		$arr_f = $this->Info_activity_model->get_state_activity('1','type,id',$index_id);
		$arr['flash_id'] = '';
        ...
        if ($this->Info_activity_model->activity_case($arr,$index_id)) {
	...
	public function activity_c_img_up(){
		...
		$img_www = '';
                ...
                $img_www = $this->Info_activity_model->rfind($map);
		$action = $_GET['action'];
		switch ($action) {
		    case 'config':
