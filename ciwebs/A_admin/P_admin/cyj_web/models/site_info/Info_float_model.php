...
class Info_float_model extends MY_Model {
    ...
    public function upfloat($arr,$id,$type){
        $this->db->trans_strict(FALSE);
        $this->db->trans_begin();
        //$this->db->insert('info_material',$arr);//素材表，暂时不启用
        if($type == 1){
            $arr_e['img_A'] = $arr['img_url'];
        }else if($type == 2){
            $arr_e['img_B'] = $arr['img_url'];
        }
        $this->db->where('id',$id);
        $this->db->update('info_float_list_edit',$arr_e);
