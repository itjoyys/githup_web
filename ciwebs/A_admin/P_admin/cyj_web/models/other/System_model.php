	...
	public function get_sp_limit($aid) {
	    ...
	    if (empty($vdata)) {
	        ...
	        foreach ($spArr as $key => $val) {
	            $data[$val] = $mobj->join("inner join k_user_agent_sport_set on sp_games_view.id = k_user_agent_sport_set.type_id")->where("k_user_agent_sport_set.is_default = '".$is_default."' and sp_games_view.type = '".$val."' and k_user_agent_sport_set.site_id = '".$_SESSION['site_id']."' and k_user_agent_sport_set.aid = '".$aid."'")->select();
