...
class Report_model extends MY_Model {
        	...
        	switch ($val) {
        		...
        		case 'bbin':
        			...
        			if ($val == 'bbin') {
                        ...
                        $data['agter'] = $this->get_report_video($date,$aids,$val,'agter',$gid);
	...
	public function get_report_fc($date =array(),$agent_id ='',$type ='',$gid){
        ...
        if (!empty($agent_id)) {
            ...
            $map['agent_id'] = array('>',0);
        }

        //结算时间
        //$map['addtime'] = array(array('>=',$date[0]),array('<=',$date[1]));
	...
	public function get_report_sp($date =array(),$agent_id ='',$type ='',$gid){
        ...
        //结算时间bet_time
        //$map['bet_time'] = array(array('>=',$date[0]),array('<=',$date[1]));
        $map['update_time'] = array(array('>=',$date[0]),array('<=',$date[1]));
        $map['site_id'] = $_SESSION['site_id'];
        $map['is_jiesuan'] = 1;
	...
	public function get_report_video($date,$aid,$type,$game_type,$gid){
        ...
        if ($type == 'bbin') {
            ...
            $map['result_type'] = array('in','(1,2)');
            $date[0] = date("Y-m-d H:i:s",strtotime("$date[0] +12 hour"));
            $date[1] = date("Y-m-d H:i:s",strtotime("$date[1] +12 hour"));
        }elseif($type == 'ct'){
            $map['is_revocation'] = 1;
            $date[0] = date("Y-m-d H:i:s", strtotime("$date[0] +11 hours"));
            $date[1] = date("Y-m-d H:i:s", strtotime("$date[1] +11 hours"));
        }elseif($type == 'lebo'){
            //有效注单
            $map['is_revocation'] = 0;
        }elseif($type == 'pt'){
            ...
            if ($game_type == 'agter') {
                $map['game_type'] = 'HUNTER';//捕鱼
                $field_name = array('bet_time','bet_amount','valid_betamount','netamount+jackpot_settlement','game_type');
            }else{
                $map['game_type'] = array('<>','HUNTER');//去掉捕鱼
            }
        }
    ...
    public function get_agent_idu($pid){
        ...
        $map['is_demo'] = 0;
        $map['agent_type'] = 'a_t';
        return $this->M(array('tab'=>'k_user_agent','type'=>1))->field("id")->where($map)->select("u-id");
    }

    //获取站点旗下所有代理
    public function get_all_agents($pid){
        $map = array();
        if ($pid) {
            $map['pid'] = $pid;
        }
        ...
        $map['is_demo'] = 0;
        $map['agent_type'] = 'a_t';
        return $this->M(array('tab'=>'k_user_agent','type'=>1))->field("id,site_id,agent_type,is_demo")->where($map)->select("id");
    }
