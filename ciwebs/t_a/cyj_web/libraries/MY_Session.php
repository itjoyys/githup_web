...
Class MY_Session Extends CI_Session {
	...
	public function __construct($params = array()) {
		if (isset($_SESSION)) {
			log_message('debug', 'A session was already started.');
			@session_destroy();
			unset($_SESSION);
		}
		$Handler = new Cache_Session_Handler;
		...
		session_start();
		log_message('debug', 'A session was started.' . print_r($_SESSION, TRUE));
		//开始
	...
	public function sess_read() {
		...
		if (!is_array($session) OR !isset($session['session_id']) OR
			...
			log_message('debug', 'A session was not found.' . print_r($session, TRUE));
		...
		if (($session['last_activity'] + $this->sess_expiration) < $this->now) {
			log_message('debug', 'A session was expiration.');
			$this->sess_destroy(FALSE);
			return FALSE;
		}
	...
	public function __construct() {
		...
		if (!$this->_CI->load->is_loaded('cache')) {
			...
			$this->_CI->load->driver(
				'cache', array('adapter' => 'file', 'backup' => 'memcached')
			);
		}
	}
