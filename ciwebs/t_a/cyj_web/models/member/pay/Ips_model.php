...
class Ips_model extends Online_api_model {
	...
	function get_all_info($url,$order_num,$s_amount,$bank,$pay_id,$pay_key){
			...
			$data['Amount'] = number_format($s_amount, 2, '.', '');//订单金额(保留2位小数)
			$data['Date'] = date('Ymd');//订单日期
			$data['Currency_Type'] = 'RMB';//币种
			$data['Gateway_Type'] = 01;//支付卡种
			$data['Lang'] = 'GB';//语言
			$data['Merchanturl'] = $ServerUrl;//支付结果成功返回的商户URL
			$data['FailUrl'] = $ServerUrl;//支付结果失败返回的商户URL
			$data['ErrorUrl'] = $ServerUrl;//支付结果错误返回的商户URL
			$data['Attach'] = '';//商户数据包
			$data['DispAmount'] = $s_amount;//显示金额
			$data['OrderEncodeType'] = 5;//订单支付接口加密方式
			$data['RetEncodeType'] = 17;//交易返回接口加密方式
			$data['Rettype'] = 1;//返回方式
			$data['ServerUrl'] =  $ServerUrl;//Server to Server 返回页面URL
			...
			$data['SignMD5'] =  md5($orge);//Server to Server 返回页面URL */
