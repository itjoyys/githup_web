...
#   获取开奖号码    
    ...
    cur.execute("select * from liuhecai_auto where qishu = '"+qishu+"' ")
    rs = cur.fetchall()
    hm  = [rs[0]['ball_1'],rs[0]['ball_2'],rs[0]['ball_3'],rs[0]['ball_4'],rs[0]['ball_5'],rs[0]['ball_6'],rs[0]['ball_7'],rs[0]['x1'],rs[0]['x2'],rs[0]['x3'],rs[0]['x4'],rs[0]['x5'],rs[0]['x6'],rs[0]['sx']]
    con.commit()
    cur.close()     
    con.close()
#   根据期数读取未结算的注单
...
#   根据期数读取未结算的注单
    ...
    cur = con.cursor ( cursorclass = MySQLdb . cursors . DictCursor )                     
    try:
        mysql="update liuhecai_auto set ok=1 where qishu="+qishu
        cur.execute(mysql)
        con.commit()
...
if __name__ == '__main__':
    con = MySQLdb.connect( DB_HOST,DB_USER,DB_PASSWD,DB_NAME,charset='utf8' )   
    cur = con.cursor ( cursorclass = MySQLdb . cursors . DictCursor )
    mysql="update liuhecai_auto set ok=0 where ok=2 order by id desc limit 10"
    cur.execute(mysql)
    con.commit() 
    cur.execute("select * from liuhecai_auto where ok=0 order by id desc limit 1 ")
    rs = cur.fetchall()
    if rs:
        mysql="update liuhecai_auto set ok=2 where qishu="+str(rs[0]['qishu'])
        cur.execute(mysql)
        con.commit()
        try:
            balance7(str(rs[0]['qishu']))  
        finally:
            if con:
                con.close() 
#end of model
