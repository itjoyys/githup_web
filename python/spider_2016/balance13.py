...
if __name__ == '__main__':
    con = MySQLdb.connect( DB_HOST,DB_USER,DB_PASSWD,DB_NAME,charset='utf8' ) 
    cur = con.cursor ( cursorclass = MySQLdb . cursors . DictCursor )
    mysql="update js_k3_auto set ok=0 where ok=2 order by id desc limit 10"
    cur.execute(mysql) 
    con.commit() 
    cur.execute("select * from js_k3_auto where ok=0 order by id desc limit 1 ")
    rs = cur.fetchall()
    if rs:
        mysql="update js_k3_auto set ok=2 where qishu="+str(rs[0]['qishu'])
        cur.execute(mysql)
        con.commit()
        try:
