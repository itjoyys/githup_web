...
#   未结算的注单
    ...
    for row in rows:
        ...
        if(row['MB_Inball']==None or row['TG_Inball']==None):
            ...
            cur.execute(mysql) 
            con.commit() 
            MB_Inball = str(rs['MB_Inball_OK'])
            TG_Inball = str(rs['TG_Inball_OK'])
        else:
...
#   未结算的串关注单
    cur.execute("select * from k_bet_cg where match_id="+rs['Match_ID']+"  and status=0")
    rows = cur.fetchall()
    if(rows):
        for row in rows:
            if(row['MB_Inball']==None or row['TG_Inball']==None):
                ...
                MB_Inball = row['MB_Inball']
                TG_Inball = row['TG_Inball']            
            if not (row['point_column'] in ['match_bmdy','match_bgdy','match_bhdy','match_bho','match_bao','match_bdpl','match_bxpl']):
...
#   根据期数读取未结算的注单
    ...
    cur = con.cursor ( cursorclass = MySQLdb . cursors . DictCursor )                      
    try:
        mysql="update lq_match set Match_JS=1 where Match_ID="+rs['Match_ID']
        cur.execute(mysql)
        con.commit()
...
if __name__ == '__main__':
    ...
    try:
        con = MySQLdb.connect( DB_HOST,DB_USER,DB_PASSWD,DB_NAME,charset='utf8' )   
        cur = con.cursor ( cursorclass = MySQLdb . cursors . DictCursor )
        mysql="update lq_match set Match_JS=0 where Match_JS=2 and (MB_Inball_OK >=0 or MB_Inball_OK =-1) order by ID desc limit 20"
        cur.execute(mysql)
        con.commit()  
        cur.execute("select * from lq_match where Match_JS =0 and (MB_Inball_OK >=0 or MB_Inball_OK =-1) order by ID desc limit 5 ")
        rs = cur.fetchall()
        if rs:        
            workerbee(rs)
