...
#   根据期数读取未结算的注单
    ...
    cur = con.cursor ( cursorclass = MySQLdb . cursors . DictCursor )                      
    try:
        mysql="update c_auto_3 set ok=1 where qishu="+qishu
        cur.execute(mysql)
        con.commit()
...
if __name__ == '__main__':
    con = MySQLdb.connect( DB_HOST,DB_USER,DB_PASSWD,DB_NAME,charset='utf8' )   
    cur = con.cursor ( cursorclass = MySQLdb . cursors . DictCursor ) 
    mysql="update c_auto_3 set ok=0 where ok=2 order by id desc limit 10"
    cur.execute(mysql)
    con.commit() 
    cur.execute("select * from c_auto_3 where ok=0 order by id desc limit 1 ")
    rs = cur.fetchall()
    if rs:
        mysql="update c_auto_3 set ok=2 where qishu="+str(rs[0]['qishu'])
        cur.execute(mysql)
        con.commit()
        try:
