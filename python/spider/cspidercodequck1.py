...
def getTitle(Uid,url,time=10):
    ...
    response.close()
    if(Uid>9 or Uid==2):
        tables = {2:'c_auto_2',10:'c_auto_10',11:'c_auto_11',12:'c_auto_12'}
...
def Cbet4(html):
    ...
    for title in html:
        con = MySQLdb.connect( DB_HOST,DB_USER,DB_PASSWD,DB_NAME,charset='utf8' )
        cur = con.cursor()
        qishu = title['expect'][2:]
        qishuint = int(qishu)
        ball = title['opencode'].split(',')
        cur.execute("select * from c_auto_4 where qishu = '"+qishu+"'")
        rows = cur.fetchall()
        if(qishuint>oldnum):
            oldnum = qishuint
            redisarray = [qishu,title['opentime'],ball[0],ball[1],ball[2],ball[3],ball[4],ball[5],ball[6],ball[7]]
        if not rows: 
            cur.execute("insert into `c_auto_4` ( qishu,datetime,ok,ball_1,ball_2,ball_3,ball_4,ball_5,ball_6,ball_7,ball_8) values ('"+qishu+"','"+title['opentime']+"',0,'"+ball[0]+"','"+ball[1]+"','"+ball[2]+"','"+ball[3]+"','"+ball[4]+"' ,'"+ball[5]+"','"+ball[6]+"','"+ball[7]+"')")                                             
            cur.execute("insert into `cq_ten_auto` ( qishu,datetime,ok,ball_1,ball_2,ball_3,ball_4,ball_5,ball_6,ball_7,ball_8) values ('"+qishu+"','"+title['opentime']+"',0,'"+ball[0]+"','"+ball[1]+"','"+ball[2]+"','"+ball[3]+"','"+ball[4]+"' ,'"+ball[5]+"','"+ball[6]+"','"+ball[7]+"')") 
            con.commit()
            cur.close()     
            con.close() 
    if  redisarray:
        ...
        RedisClent.set('c_auto_4_result',redisarray)
        print 'ZDREDIS END' 
...
def Cbet10(UId,html,table):
    ...
    for title in html:
        ...
        ball = title['opencode'].split(',')
        qishuint = int(qishu)
        cur.execute("select * from c_auto_10 where qishu = '"+qishu+"'")
        rows = cur.fetchall()
        if(qishuint>oldnum):
            oldnum = qishuint
            redisarray = [qishu,title['opentime'],ball[0],ball[1],ball[2],ball[3],ball[4]]
        if not rows: 
            cur.execute("insert into `c_auto_10` ( qishu,datetime,ok,ball_1,ball_2,ball_3,ball_4,ball_5) values ('"+qishu+"','"+title['opentime']+"',0,'"+ball[0]+"','"+ball[1]+"','"+ball[2]+"','"+ball[3]+"','"+ball[4]+"' )")                                             
            cur.execute("insert into `tj_ssc_auto` ( qishu,datetime,ok,ball_1,ball_2,ball_3,ball_4,ball_5) values ('"+qishu+"','"+title['opentime']+"',0,'"+ball[0]+"','"+ball[1]+"','"+ball[2]+"','"+ball[3]+"','"+ball[4]+"' )")  
            con.commit()
            cur.close()     
            con.close() 
    if  redisarray:
        print 'ZDREDIS START'
        redisarray = json.dumps(redisarray)
        RedisClent.set('c_auto_10_result',redisarray)
        print 'ZDREDIS END'
...
operator = {2:Cbet2,3:Cbet3,4:Cbet4,9:Cbet9,10:Cbet10,11:Cbet2,12:Cbet2}
...
#                 [3,'http://api.caipiaokong.com/lottery/?name=bjpks&format=json&uid=88687&token=89ED44DE68D3F3BECF511F0AC8FC9FDB&num=3'],#北京赛车PK拾
                # [4,'http://api.caipiaokong.com/lottery/?name=cqxync&format=json&uid=88687&token=89ED44DE68D3F3BECF511F0AC8FC9FDB&num=30'],#重庆快乐十分
                 [4,'http://c.apiplus.net/newly.do?token=6d8caf8c25bc44f5&code=cqklsf&format=json&rows='+str(num)],#重庆快乐十分
#                  [9,'http://cp.swlc.sh.cn/notice/ssl/index.html'] # 上海时时乐
#                  [10,'http://api.caipiaokong.com/lottery/?name=tjssc&format=json&uid=88687&token=89ED44DE68D3F3BECF511F0AC8FC9FDB&num=30'],  #天津时时彩
                 [10,'http://c.apiplus.net/newly.do?token=6d8caf8c25bc44f5&code=tjssc&format=json&rows='+str(num)],  #天津时时彩
#                 [11,'http://api.caipiaokong.com/lottery/?name=jxssc&format=json&uid=88687&token=89ED44DE68D3F3BECF511F0AC8FC9FDB&num=3'],  #江西时时彩
#                 [12,'http://api.caipiaokong.com/lottery/?name=xjssc&format=json&uid=88687&token=89ED44DE68D3F3BECF511F0AC8FC9FDB&num=3'],  #新疆时彩
             ]
        for row in rows:
            #print row
