...
#   获取开奖号码    
    ...
    mysql="update tennis_match set Match_JS=2 where Match_ID="+rs['Match_ID']
    cur.execute(mysql)
    con.commit()
    cur.close()     
    con.close()
...
#   根据期数读取未结算的注单
    ...
    cur = con.cursor ( cursorclass = MySQLdb . cursors . DictCursor )                      
    try:
        mysql="update tennis_match set Match_JS=1 where Match_ID="+rs['Match_ID']
        cur.execute(mysql)
        con.commit()
...
if __name__ == '__main__':
    ...
    try:
        ...
        cur.execute(mysql)
        con.commit()        
        cur.execute("select * from tennis_match where Match_JS =0 and ( MB_Inball >=0 or MB_Inball =-1 ) order by ID desc limit 5 ")
        rs = cur.fetchall()
        if rs:        
            workerbee(rs)
