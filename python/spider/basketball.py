...
if __name__ == '__main__':
    ...
    try:
        con = MySQLdb.connect( DB_HOST,DB_USER,DB_PASSWD,DB_NAME,charset='utf8' )   
        cur = con.cursor ( cursorclass = MySQLdb . cursors . DictCursor )
        mysql="update lq_match set Match_JS=0 where Match_JS=2 and (MB_Inball >=0 or MB_Inball =-1) order by ID desc limit 20"
        cur.execute(mysql)
        con.commit()  
        cur.execute("select * from lq_match where Match_JS =0 and (MB_Inball >=0 or MB_Inball =-1) order by ID desc limit 5 ")
        rs = cur.fetchall()
        if rs:        
            workerbee(rs)
