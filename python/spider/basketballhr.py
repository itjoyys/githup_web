    ...
    if(rows):
        for row in rows:
            if not (row['point_column'] in ['match_bmdy','match_bgdy','match_bhdy','match_bho','match_bao','match_bdpl','match_bxpl']):
                t = sportsalgorithm.make_point(row['point_column'].lower(), row['MB_Inball'], row['TG_Inball'], '','', '', row['match_showtype'], row['match_rgg'], row['match_dxgg'], row['match_nowscore'])
            else:
                t = sportsalgorithm.make_point(row['point_column'].lower(),'' , '', row['MB_Inball'],row['TG_Inball'], '', row['match_showtype'], row['match_rgg'], row['match_dxgg'], row['match_nowscore'])
####################################################结算单式##################################################
...
if __name__ == '__main__':
    ...
    try:
        con = MySQLdb.connect( DB_HOST,DB_USER,DB_PASSWD,DB_NAME,charset='utf8' )   
        cur = con.cursor ( cursorclass = MySQLdb . cursors . DictCursor )
        mysql="update lq_match set Match_JS=0 where Match_JS=2 and (MB_Inball_OK >=0 or MB_Inball_OK =-1) and MB_Inball is null order by ID desc limit 20"
        cur.execute(mysql) 
        con.commit() 
        cur.execute("select * from lq_match where Match_JS =0 and (MB_Inball_OK >=0 or MB_Inball_OK =-1) and MB_Inball is null order by ID desc limit 5 ")
        rs = cur.fetchall()
        if rs:        
            workerbee(rs)
