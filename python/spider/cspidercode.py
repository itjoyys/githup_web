...
#         cur.execute("select * from c_auto_1 where qishu = '20"+qishu[0]+"'")
#         rows = cur.fetchall()
#         if not rows: 
#             cur.execute("insert into `c_auto_1` ( qishu,datetime,ok,ball_1,ball_2,ball_3,ball_4,ball_5,ball_6,ball_7,ball_8) values ('20"+qishu[0]+"','"+timec[0]+"','"+status+"','"+ball[0]+"','"+ball[1]+"','"+ball[2]+"','"+ball[3]+"','"+ball[4]+"','"+ball[5]+"','"+ball[6]+"','"+ball[7]+"' )")                                             
#             con.commit()
#             cur.close()     
#             con.close()
...
def Cbet1(html):
    ...
    for title in html:
        ...
        cur.execute("select * from c_auto_1 where qishu = '"+qishu+"'")
        rows = cur.fetchall()
        if(qishuint>oldnum):
            oldnum = qishuint
            redisarray = [qishu,title['opentime'],ball[0],ball[1],ball[2],ball[3],ball[4],ball[5],ball[6],ball[7]]
        if not rows: 
            cur.execute("insert into `c_auto_1` ( qishu,datetime,ok,ball_1,ball_2,ball_3,ball_4,ball_5,ball_6,ball_7,ball_8) values ('"+qishu+"','"+title['opentime']+"',0,'"+ball[0]+"','"+ball[1]+"','"+ball[2]+"','"+ball[3]+"','"+ball[4]+"','"+ball[5]+"','"+ball[6]+"','"+ball[7]+"' )")                                             
    ...
    if  redisarray:
        ...
        RedisClent.set('c_auto_1_result',redisarray)
        print 'ZDREDIS END'
...
#         cur.execute("select * from c_auto_1 where qishu = '"+title+"'")
#         rows = cur.fetchall()
#         if not rows: 
#             cur.execute("insert into `c_auto_1` ( qishu,datetime,ok,ball_1,ball_2,ball_3,ball_4,ball_5,ball_6,ball_7,ball_8) values ('"+title+"','"+html[title]['dateline']+"',0,'"+ball[0]+"','"+ball[1]+"','"+ball[2]+"','"+ball[3]+"','"+ball[4]+"','"+ball[5]+"','"+ball[6]+"','"+ball[7]+"' )")                                             
#             con.commit()
#             cur.close()     
#             con.close()
...
def Cbet3(html): 
    ...
    #         timenow=int(time.time())
            ...
            sql="select * from c_auto_3 where qishu = '"+val+"'"
            #print sql
            try:
                cur.execute(sql)
                rows = cur.fetchall()
            ...
            #print rows[0]
            if not rows:
                insert_sql="insert into `c_auto_3` ( qishu,datetime,ok,ball_1,ball_2,ball_3,ball_4,ball_5,ball_6,ball_7,ball_8,ball_9,ball_10 ) values ('"+val+"','"+open_time+"','"+status+"','"+ball[0]+"','"+ball[1]+"','"+ball[2]+"','"+ball[3]+"','"+ball[4]+"','"+ball[5]+"','"+ball[6]+"','"+ball[7]+"','"+ball[8]+"','"+ball[9]+"' )"
                print insert_sql
...
def Cbet5(html): 
    ...
    if not ball[0]=='-':   
        ...
        RedisClent.set('c_auto_5_result',redisarray)
        print 'ZDREDIS END'       
...
def Cbet6(html):
    ...
    titleredis=[qishu[0],timec[0],ball[0],ball[1],ball[2]]
    redisarray = json.dumps(titleredis)
    RedisClent.set('c_auto_6_result',redisarray)
    print 'ZDREDIS END' 
    if not rows: 
...
#     cur = con.cursor()
#     status ='0'
#     cur.execute("select * from c_auto_7 where nn = '"+str(qishu)+"'")
#     rows = cur.fetchall()
...
#     if not rows: 
#         cur.execute("insert into `c_auto_7` ( nn,nd,na,n1,n2,n3,n4,n5,n6,ok,sx,x1,x2,x3,x4,x5,x6) values ('"+qishu+"','"+timecor+"','"+ball[6]+"','"+ball[0]+"','"+ball[1]+"','"+ball[2]+"','"+ball[3]+"','"+ball[4]+"','"+ball[5]+"','"+status+"','"+zodiacs[6]+"','"+zodiacs[0]+"','"+zodiacs[1]+"','"+zodiacs[2]+"','"+zodiacs[3]+"','"+zodiacs[4]+"','"+zodiacs[5]+"' )")                                             
#         con.commit()
#         cur.close()     
#         con.close()
...
#             status ='0'
#             print data_date+':'+val+" open_time:"+open_time
#             sql="select * from c_auto_8 where qishu = '"+val+"'"
#             print sql
#             try:
#                 cur.execute(sql)
#                 rows = cur.fetchone()
...
def Cbet8(html):
    ...
    if  redisarray:
        print 'ZDREDIS START'
        redisarray = json.dumps(redisarray)
        RedisClent.set('c_auto_8_result',redisarray)
        print 'ZDREDIS END'                    
# def Cbet8(html):
#     infoencode = chardet.detect(html).get('encoding','utf-8')
#     html = html.decode(infoencode,'ignore').encode('utf-8')
...
#         cur = con.cursor()
#         ball = html[title]['number'].split(',')
#         cur.execute("select * from c_auto_8 where qishu = '"+title+"'")
#         rows = cur.fetchall()
#         if not rows: 
