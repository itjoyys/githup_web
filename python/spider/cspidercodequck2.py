...
def Cbet2(UId,html,table):
    ...
    for title in html:
        ...
        if not rows: 
            ...
            if(table == 'c_auto_11'):
                tablenew = 'jx_ssc_auto'
            elif(table == 'c_auto_12'):
                tablenew = 'xj_ssc_auto'
            cur.execute("insert into "+tablenew+" ( qishu,datetime,ok,ball_1,ball_2,ball_3,ball_4,ball_5) values ('"+qishu+"','"+title['opentime']+"',0,'"+ball[0]+"','"+ball[1]+"','"+ball[2]+"','"+ball[3]+"','"+ball[4]+"' )")                                         
            con.commit()
            cur.close()     
            con.close() 
