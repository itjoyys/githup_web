...
def Cbet2(UId,html,table):
    ...
    for title in html:
        ...
        if not rows: 
            cur.execute("insert into `c_auto_2` ( qishu,datetime,ok,ball_1,ball_2,ball_3,ball_4,ball_5) values ('"+qishu+"','"+title['opentime']+"',0,'"+ball[0]+"','"+ball[1]+"','"+ball[2]+"','"+ball[3]+"','"+ball[4]+"' )")                                             
            cur.execute("insert into `cq_ssc_auto` ( qishu,datetime,ok,ball_1,ball_2,ball_3,ball_4,ball_5) values ('"+qishu+"','"+title['opentime']+"',0,'"+ball[0]+"','"+ball[1]+"','"+ball[2]+"','"+ball[3]+"','"+ball[4]+"' )")   
            con.commit()
            cur.close()     
            con.close() 
    ...
    if  redisarray:
        ...
        RedisClent.set('c_auto_2_result',redisarray)
        print 'ZDREDIS END'
...
def Cbet3(html):
    ...
    for title in html:
        ...
        qishuint = int(qishu)
        ball = title['opencode'].split(',')
        cur.execute("select * from c_auto_3 where qishu = '"+qishu+"'")
        rows = cur.fetchall()
        if(qishuint>oldnum):
            oldnum = qishuint
            redisarray = [qishu,title['opentime'],ball[0],ball[1],ball[2],ball[3],ball[4],ball[5],ball[6],ball[7],ball[8],ball[9]]
        if not rows: 
            cur.execute("insert into `c_auto_3` ( qishu,datetime,ok,ball_1,ball_2,ball_3,ball_4,ball_5,ball_6,ball_7,ball_8,ball_9,ball_10) values ('"+qishu+"','"+title['opentime']+"',0,'"+ball[0]+"','"+ball[1]+"','"+ball[2]+"','"+ball[3]+"','"+ball[4]+"','"+ball[5]+"','"+ball[6]+"','"+ball[7]+"','"+ball[8]+"','"+ball[9]+"'  )")                                             
            cur.execute("insert into `bj_10_auto` ( qishu,datetime,ok,ball_1,ball_2,ball_3,ball_4,ball_5,ball_6,ball_7,ball_8,ball_9,ball_10) values ('"+qishu+"','"+title['opentime']+"',0,'"+ball[0]+"','"+ball[1]+"','"+ball[2]+"','"+ball[3]+"','"+ball[4]+"','"+ball[5]+"','"+ball[6]+"','"+ball[7]+"','"+ball[8]+"','"+ball[9]+"'  )")             
            con.commit()
            cur.close()     
            con.close()
    if  redisarray:
        print 'ZDREDIS START'
        redisarray = json.dumps(redisarray)
        RedisClent.set('c_auto_3_result',redisarray)
        print 'ZDREDIS END'   
...
def run():
 ...
 #                 [2,'http://baidu.lecai.com/lottery/draw/ajax_get_latest_draw_html.php?lottery_type=200'],
                 #[2,'http://api.caipiaokong.com/lottery/?name=cqssc&format=json&uid=88687&token=89ED44DE68D3F3BECF511F0AC8FC9FDB&num=30'],#重庆时时彩
                 [2,'http://c.apiplus.net/newly.do?token=6d8caf8c25bc44f5&code=cqssc&format=json&rows='+str(num)],#重庆时时彩
                #[3,'http://api.caipiaokong.com/lottery/?name=bjpks&format=json&uid=88687&token=89ED44DE68D3F3BECF511F0AC8FC9FDB&num=30'],#北京赛车PK拾
                  [3,'http://c.apiplus.net/newly.do?token=6d8caf8c25bc44f5&code=bjpk10&format=json&rows='+str(num)],#北京赛车PK拾
