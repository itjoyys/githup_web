...
def getTitle(Uid,url,time=10):
    ...
    print Uid
    response.close()
    tables = {13:'c_auto_13',14:'c_auto_14'}
...
def Cbet11(html,table):
    ...
    for title in html:
        ...
        if not rows: 
            ...
            if(table == 'c_auto_13'):
                tablenew = 'js_k3_auto'
            elif(table == 'c_auto_14'):
                tablenew = 'jl_k3_auto'
            cur.execute("insert into "+tablenew+" (  qishu,datetime,ok,ball_1,ball_2,ball_3 ) values ('"+qishu+"','"+title['opentime']+"',0,'"+ball[0]+"','"+ball[1]+"','"+ball[2]+"' )")                                                            
            con.commit()
            cur.close()     
            con.close()
