...
def win_cbet(row):
    ...
    try:
        ...
        users = cur.fetchall()                    
        user =users[0]                   
        sql = " insert into k_user_cash_record (site_id ,source_id,username,uid,agent_id,cash_type,cash_do_type,cash_num,cash_balance,cash_date,remark,discount_num,index_id) values ('"+user['site_id']+"','"+str(row['id'])+"','"+str(user['username'])+"','"+str(row['uid'])+"','"+str(row['agent_id'])+"',14,1,'"+str(row['win'])+"',"+str(user['money'])+unicode(",now(),'自动结算单号："+str(row['did'])+" 類型：",'utf-8')+row['type']+"',0,'"+str(user['index_id'])+"') "
        cur.execute(sql)
...
def invalid_cbet(row):
    ...
    try:
        ...
        users = cur.fetchall()                    
        user =users[0]                   
        sql = " insert into k_user_cash_record (site_id ,source_id,username,uid,agent_id,cash_type,cash_do_type,cash_num,cash_balance,cash_date,remark,discount_num,index_id) values ('"+user['site_id']+"','"+str(row['id'])+"','"+str(user['username'])+"','"+str(row['uid'])+"','"+str(row['agent_id'])+"',20,1,'"+str(row['money'])+"',"+str(user['money'])+unicode(",now(),'自动结算单号："+str(row['did'])+" 類型：",'utf-8')+row['type']+"',0,'"+str(user['index_id'])+"') "
        cur.execute(sql)
    except MySQLdb.Error, e:
        con.rollback()
        print "Error %d: %s" % (e.args[0],e.args[1])                      
