...
#   获取开奖号码    
    ...
    mysql="update bet_match set Match_JS=2 where Match_ID="+rs['Match_ID']
    cur.execute(mysql)
    con.commit()
    cur.close()     
    con.close()
#   未结算的注单
    ...
    for row in rows:
        ...
        t = sportsalgorithm.make_point(row['point_column'].lower(), MB_Inball, TG_Inball, MB_Inball_HR,TG_Inball_HR, row['match_type'], row['match_showtype'], row['match_rgg'], row['match_dxgg'], row['match_nowscore'])
        print t
...
#   未结算的串关注单
    cur.execute("select * from k_bet_cg where match_id="+rs['Match_ID']+" and status=0 and point_column in ( 'match_ho','match_ao','match_dxdpl','match_dxxpl','match_bd10','match_bd20','match_bd21','match_bd30','match_bd31','match_bd32','match_bd40','match_bd41','match_bd42','match_bd43','match_bdg10','match_bdg20','match_bdg21','match_bdg30','match_bdg31','match_bdg32','match_bdg40','match_bdg41','match_bdg42','match_bdg43','match_bd00','match_bd11','match_bd22','match_bd33','match_bd44','match_bdup5','match_total01pl','match_total23pl','match_total46pl','match_total7uppl','match_bzm','match_bzg','match_bzh','match_bqmm','match_bqmh','match_bqmg','match_bqhm','match_bqhh','match_bqhg','match_bqgm','match_bqgh','match_bqgg','match_dsdpl','match_dsspl' ) ")
    rows = cur.fetchall()
    if(rows):
        for row in rows:
            if(row['MB_Inball']==None or row['TG_Inball']==None):
                mysql = "Update `k_bet_cg` set MB_Inball='"+str(rs['MB_Inball'])+"',TG_Inball='"+str(rs['TG_Inball'])+"' where bid = "+str(row['bid']) 
                cur.execute(mysql) 
                con.commit() 
                MB_Inball = str(rs['MB_Inball'])
                TG_Inball = str(rs['TG_Inball'])            
                ...
                MB_Inball = row['MB_Inball']
                TG_Inball = row['TG_Inball']            
            t = sportsalgorithm.make_point(row['point_column'].lower(), MB_Inball, TG_Inball, '','', '', row['match_showtype'], row['match_rgg'], row['match_dxgg'], row['match_nowscore'])
####################################################结算单式##################################################
...
#   根据期数读取未结算的注单
    ...
    cur = con.cursor ( cursorclass = MySQLdb . cursors . DictCursor )                      
    try:
        mysql="update bet_match set Match_JS=1 where Match_ID="+rs['Match_ID']
        cur.execute(mysql)
        con.commit()
...
if __name__ == '__main__':
    ...
    try:
        con = MySQLdb.connect( DB_HOST,DB_USER,DB_PASSWD,DB_NAME,charset='utf8' )   
        cur = con.cursor ( cursorclass = MySQLdb . cursors . DictCursor )
        mysql="update bet_match set Match_JS=0 where Match_JS=2 and ( MB_Inball >=0 or MB_Inball =-1 ) order by ID desc limit 20"
        cur.execute(mysql) 
        con.commit()
        cur.execute("select * from bet_match where Match_JS =0 and ( MB_Inball >=0 or MB_Inball =-1 ) order by ID desc limit 5 ")
        rs = cur.fetchall()
        if rs:        
            workerbee(rs)
