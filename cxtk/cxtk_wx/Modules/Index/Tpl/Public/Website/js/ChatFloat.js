        ...
        var ajax = new Ajax(function (responseText, responseXML) {
            if (responseText) {
                ...
                if (obj && obj.weimobid && obj.weimobid.length > 0) {
                    t.initialize(options, obj.weimobid, obj.msgcount);
                    e.setCookie('useright', '{"aid":' + options.AId + ',"weimobid":"' + obj.weimobid + '"}');
                }
            }
        }, function (status) { });
    ...
    if (options && options.AId) {
        ...
        if (strStoreData && strStoreData.length > 0) {
            ...
            if (objStorage && objStorage.AId == options.AId && objStorage.weimobid) {
                e.initialize(options, objStorage.weimobid, 0);
...
ChatFloat.prototype = {
    initialize: function (options, weimobid, msgcount) {
        var e = this;
    ...
    socketconnect: function (options, weimobid) {
        try {
            ...
            socket.on('disconnect', function () { Tools.StartPolling(options.AId, weimobid); socket = null; });
        }
        catch (e) {
            Tools.StartPolling(options.AId, weimobid);
        }
    }
};
