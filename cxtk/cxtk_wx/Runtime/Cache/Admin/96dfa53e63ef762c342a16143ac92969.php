...
<script type="text/javascript">
  ...
  <div class="content-box role">
    <div class="content-box-header">
      <h3>表单字段<a class="add_cost" href="add_web_flash">添加字段</a>
      <a class="add_cost" href="add_web_flash">表单数据</a>
    </h3>
      <div class="clear"></div>
    </div>
      ...
      <table class="table">
       <thead>
        <tr>
          ...
          <th><div class="header">类型</div></th>
          <th><div class="header">是否必填</div></th>
          <th style="width:130px;"><div class="header">添加时间</div></th>
          <th style="width:260px;"><div class="header" >操作</div></th>

        </tr>
       </thead>
        <?php if(is_array($data)): foreach($data as $key=>$v): ?><tr <?php if(($key%2)==0): ?>class="td_one"<?php endif; ?>>
               ...
               <td style="text-align:center;"><?php if(empty($v['img'])): ?><font style="color:red;" class="view" onclick="img_up('<?php echo ($v['id']); ?>')">【上传】
                ...
                <?php else: ?>
                     <font style="color:#1A6102;" class="view" onclick="view('<?php echo ($v['img']); ?>','<?php echo ($v['width']); ?>','<?php echo ($v['height']); ?>')">【预览】<?php endif; ?>
              ...
              <td style="width:85px;text-align: center;">
               ...
               <td class="td" style="text-align: center;">
                   <a href="web_flash_edit?id=<?php echo ($v['id']); ?>">修改</a>
                    <a style="background-color: #e86829;color:#fff;"  onclick="return confirm('确定删除');"
                    href="web_flash_del?id=<?php echo ($v['id']); ?>&type=<?php echo ($v['type']); ?>">删除</a>
