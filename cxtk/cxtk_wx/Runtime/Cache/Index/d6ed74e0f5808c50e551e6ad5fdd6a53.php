...
font-weight: 100;}
  ...
  li.zc { float: left; width:50%; margin: 5px 0 0 0; text-align: center; overflow: hidden; }
.zz { width:100%inherit; overflow: hidden;margin: 0 auto; -webkit-box-shadow: 0px 1px 1px #716F70;  background: #ffffff; overflow: hidden;height: }
.zz img { margin: 0 auto; }
.zz a{padding: 5px;}
.zz span { font-size: 10px; margin-left: 10px; width:95%;display:block;line-height: 18px;
height: 36px;
text-align: justify;
padding: 2px;
overflow: hidden;}
.zz .p { color: #f41051; font-size: 8px; }
.topicss li { padding-bottom: 10px; }
...
<div id="header">
	...
	<div id="banner" >
		...
		<div class="container">
				...
				<div class="meau_nav">
					<ul class="meau">
					   <?php if(is_array($cate)): $i = 0; $__LIST__ = $cate;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><li class="meau_li">
							<a href="<?php echo U(GROUP_NAME . '/List/Index', array('id' => $v['id']));?>" target="_blank"><?php echo ($v["name"]); ?></a>
            ...
            <!--新品上架-->
			<section class="box">
				<div class="tit_th"> <i class="icon_1"></i>
					<h2>新品上架</h2>
				</div>
				<div class="tb list">
	        		 ...
	        		 <?php if(is_array($newgoods)): $i = 0; $__LIST__ = $newgoods;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><li class="zc" >
						<div class="zz">
							<a href="<?php echo U(GROUP_NAME . '/Detail/Index', array('goodsid' => $v['goodsid']));?>">
								<img src="<?php echo ($image_url); echo ($v["photo1"]); ?>" class="lazy" width="100%" >

								<span><?php echo ($v["name"]); ?></span>
								<span style="height:18px;"> <b class="p">￥<?php echo ($v["saleprice"]); ?>元</b>
								</span>
