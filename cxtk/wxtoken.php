...
$keywordsql = "SELECT * FROM comm_microkeyword where siteid='$tokenid'";
$result_t = mysql_query($tokensql,$conn);
$result_c = mysql_query($concernsql,$conn);
$result_k = mysql_query($keywordsql,$conn);
$row = mysql_fetch_row($result_t);
$concern = mysql_fetch_array($result_c);
$keyword = mysql_fetch_row($result_k);
$a=mysql_num_rows($result_c);

                    $msg[0]['title'] = '宋宴官府菜';
				    $msg[0]['intro'] = '$fu';
				    $msg[0]['pic'] ='http://songyan.wx0571.com/images/songyan.jpg' ; //图片URL 这里是返回发送的原图 URL 要使用绝对完整地址
			     	$msg[0]['url'] = 'http://songyan.wx0571.com/index.html'; //这里是超链接
...
define('TOKEN',$row[0]); //定义TOKEN 常量 在微信公众平台上验证的时候就填写这个值 imtoken
echo TOKEN;
