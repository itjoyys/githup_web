...
func NewWithWriter(out io.Writer) *Macaron {
	m := &Macaron{
		Injector: inject.New(),
		action:   func() {},
		Router:   NewRouter(),
...
func NewWithLogger(newlog *log.Logger) *Macaron {
	m := &Macaron{
		Injector: inject.New(),
		action:   func() {},
		Router:   NewRouter(),
...
// Use this method if you want to have full control over the middleware that is used.
func New(newlog *log.Logger) *Macaron {
	return NewWithLogger(newlog)
}
...
func GetDefaultListenInfo() (string, int) {
	...
	port := StrTo(os.Getenv("PORT")).MustInt()
	if port == 0 {
		port = 4000
	}
	return host, port
