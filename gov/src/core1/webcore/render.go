...
type (
	...
	Render interface {
		...
		Echo(status int, body string)
		JS(status int, name string, v interface{})
		Txt(status int, filepath string)
...
//充当 从缓存中读取html
func (r *TplRender) Echo(status int, body string) {
	// json rendered fine, write out the result
...
func (r *TplRender) Txt(status int, filepath string) {
	...
	// json rendered fine, write out the result
	r.Header().Set(ContentType, ContentTXT+r.CompiledCharset)
	r.WriteHeader(status)
	r.Write(fd)
}

func (r *TplRender) JS(status int, name string, binding interface{}) {
	t := r.templateSet.Get(_DEFAULT_TPL_SET_NAME)
