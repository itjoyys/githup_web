...
func Logger() Handler {
	return func(ctx *Context, log *log.Logger) {
		...
		//如果是静态文件跳过
		str_url := ctx.Req.RequestURI
		urls := strings.Split(ctx.Req.RequestURI, "?")
		if i := len(urls); i > 1 {
			str_url = urls[0]
		}
		urls = strings.Split(str_url, ".")
		if i := len(urls); i > 1 {
			switch strings.ToUpper(urls[i-1]) {
			case "JPG", "JPEG", "GIF", "PNG", "ICO", "BMP", "CSS", "JS", "SWF", "WOFF":
				return
			}
		}
		if ColorLog {
