	...
	if reflect.TypeOf(reflect.Indirect(reflect.ValueOf(output)).Interface()).Kind() == reflect.Slice {
		...
		if tt.Kind() == reflect.Ptr {
			...
			for i := 0; i < sliceElementType.NumField(); i++ {
				...
				pk := getPKFromTag(bb, strings.ToLower(sliceElementType.Field(i).Name))
				if len(pk) > 0 {
					orm.PrimaryKey = pk
					break
				}
			...
			for i := 0; i < tt.NumField(); i++ {
				bb := tt.Field(i).Tag
				pk := getPKFromTag(bb, strings.ToLower(tt.Field(i).Name))
				if len(pk) > 0 {
					orm.PrimaryKey = pk
					break
				}
...
// @mebar
// 2015.3.12 兼容 users := make([]*User, 0, num) 和 users := make([]User, 0, num)
func (orm *Model) FindAll(rowsSlicePtr interface{}) error {
	orm.ScanPK(rowsSlicePtr) //---1
	sliceValue := reflect.Indirect(reflect.ValueOf(rowsSlicePtr))
	if sliceValue.Kind() != reflect.Slice {
		return errors.New("needs a pointer to a slice")
	}
	sliceElementType := sliceValue.Type().Elem()
	tabletype, err := getBeanType(sliceElementType)
	if err != nil {
		return err
	}
	st := reflect.New(tabletype)
	var keys []string
	...
	if orm.TableName == "" {
		orm.TableName = getTableName(rowsSlicePtr) //2
	}
	// If we've already specific columns with Select(), use that
	if orm.ColumnStr == "*" {
		for key, _ := range results {
	...
	for _, results := range resultsSlice {
		newValue := reflect.New(tabletype)
...
func (orm *Model) FindMap() (resultsSlice []map[string][]byte, err error) {
	...
	for res.Next() {
		...
		for ii, key := range fields {
			...
			//时间类型
			case reflect.Struct:
				str = rawValue.Interface().(time.Time).Format("2006-01-02 15:04:05")
				result[key] = []byte(str)
			case reflect.Bool:
				if vv.Bool() {
					result[key] = []byte("1")
...
func (orm *Model) InsertBatch1(rows []map[string]interface{}) ([]map[string]interface{}, error) {
	rrows := make([]map[string]interface{}, 0)
	tablename := orm.TableName
	if len(rows) <= 0 {
		return rrows, nil
	}
	...
	for i := 0; i < len(rows); i++ {
		orm.TableName = tablename
		_, err := orm.Insert(rows[i])
		if err != nil {
			lasterr = err
			rrows = append(rows, rows[i])
			continue
			//return ids, err
		}
	}
	return rrows, lasterr
}
