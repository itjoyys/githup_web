...
func (p *MysqlProvider) Read(sid string) (session.RawStore, error) {
	...
	if err == sql.ErrNoRows {
		_, err = p.c.Exec("INSERT INTO session(`key`,data,islogin,expiry) VALUES(?,?,?,?)",
			sid, "", 0, time.Now().Unix())
	}
	if err != nil {
		return nil, err
...
func (p *MysqlProvider) SetLogin(sid string, islogin bool) error {
	if !p.Exist(sid) {
		return fmt.Errorf("new sid '%s' not exists", sid)
	}
	var login int8 = 0
	if islogin {
		login = 1
	}
	if _, err := p.c.Exec("UPDATE session SET islogin=? WHERE `key`=?", login, sid); err != nil {
		return err
	}
	return nil
}
...
func (p *MysqlProvider) Regenerate(oldsid, sid string) (_ session.RawStore, err error) {
	...
	if _, err = p.c.Exec("UPDATE session SET `key`=?,islogin=? WHERE `key`=?", sid, oldsid); err != nil {
		return nil, err
	}
...
func (p *MysqlProvider) GC() {
	p.c.Exec("DELETE FROM session WHERE UNIX_TIMESTAMP(NOW()) - expiry > ? AND islogin = 0 ", p.expire)
}
