...
//添加用户
func Aba_CreateMember(ctx *middleware.Context) {
	//获取参数
	...
	tierule_int, _ := utility.StrTo(mv.Get("tierule")).Int()
	ratio_int, _ := utility.StrTo(mv.Get("ratio")).Int()
	...
	//是否已经纯在
	if maba.IsUserExist(username, siteid) {
	...
	for {
		...
		b = maba.IsGUserNotExist(username)
		if !b {
			username = CreateUserName(username)
		}
		ii++
	}
	aba_g, err := aba.AbaInit(siteid2)
	if err != nil {
		ctx.JSON(200, JsonResult{Result: false, Data: JsonData{Code: 10004}})
		return
	}
	...
	ii = 0
	for {
		if len(session) > 0 || ii > 4 {
			break
		}

		session, err = aba_g.LoginChk(username, user.Password, tierule_int, ratio_int)
		if err != nil || len(session) == 0 {
			if err.Error() == "INVALID_ACCOUNT" || err.Error() == "ILLEGAL_ACCOUNT_FORMAT" {
				username = CreateUserName(username)
			} else {
				common.Log.Info("aba CheckAndCreateAccount sql error：", err)
...
//进入游戏
func Aba_TransferGame(ctx *middleware.Context) {
	params := ctx.Query("params")
	params = strings.Replace(params, " ", "+", -1) //base64bug
	//解码
	siteid := strings.Replace(ctx.Req.UserAgent(), "Game_Go_", "", -1)
	...
	//获取用户
	userinfo, err := maba.GetUserInfo(username, siteid2)
	if err != nil {
	...
	html := aba_g.Getloginurl(userinfo.GUserName, userinfo.Sessionid)
