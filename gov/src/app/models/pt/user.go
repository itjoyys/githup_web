...
func Password() string {
	return string(utility.RandomCreateBytes(10))
}
...
func CreateUser(user *User) error {
	...
	if err == nil {
		user_v := fmt.Sprintf("%v'%v'%v'%v'%v'%v'%v'%v'%v", user.UserName, user.AgentId, user.Password, user.GUserName, user.SiteId, user.Balance, user.Cur, user.CreateTime, user.IndexId)
		sc := m.Redisclient.MSet("user:pt:"+user.GUserName, user_v)
		if sc.Err() != nil {
			common.Log.Err("Redisclient MSet:", sc.Err(), user)
			m.Redis_user_error[4] = 1
		}
	}
	return err
}
...
func GetUserByGname(gnames []string) (map[string]User, error) {
	...
	for _, v := range gnames {
		ptname = append(ptname, "user:pt:"+strings.ToLower(v))
	}
	users := make([]User, 0)
	rc := m.Redisclient.MGet(ptname...)
	if len(rc.Val()) > 0 {
...
func ChengUserPwd(username, siteid, pwd string) (bool, error) {

	sql := "update pt_user set password = ? where username = ? AND site_id = ?"

	_, err := m.Orm.Exec(sql, pwd, username, siteid)
	if err != nil {
...
func GetUserInfo(username, siteid string) (*User, error) {
	...
	user_v := fmt.Sprintf("%v'%v'%v'%v'%v'%v'%v'%v", user.UserName, user.AgentId, user.Password, user.GUserName, user.SiteId, user.Balance, user.Cur, user.CreateTime, user.IndexId)
	m.Redisclient.MSet("user:pt:"+user.GUserName, user_v)
	return user, nil
}
...
//加载用户到redis中
func PT_LoadAllUserInRedis() error {
	pagesize := 300
	i := 0
	hasuser := true
	for {
		...
		if count > 0 {
			for _, v := range users {
				guser = append(guser, "user:pt:"+v.GUserName)
				user_v := fmt.Sprintf("%v'%v'%v'%v'%v'%v'%v'%v'%v", v.UserName, v.AgentId, v.Password, v.GUserName, v.SiteId, v.Balance, v.Cur, v.CreateTime, v.IndexId)
