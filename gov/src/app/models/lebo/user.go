...
func CreateUser(user *User) error {
	...
	if err == nil {
		...
		sc := m.Redisclient.MSet("user:lebo:"+user.GUserName, user_v)
		if sc.Err() != nil {
			common.Log.Err("Redisclient MSet:", sc.Err(), user)
			m.Redis_user_error[3] = 1
		}
	}
	return err
}
...
func GetUserByGname(gnames []string) (map[string]User, error) {
	...
	for _, v := range gnames {
		mgname = append(mgname, "user:lebo:"+v)
	}
	users := make([]User, 0)
...
func GetUserInfo(username, siteid string) (*User, error) {
	...
	user_v := fmt.Sprintf("%v'%v'%v'%v'%v'%v'%v'%v", user.UserName, user.AgentId, user.Password, user.GUserName, user.SiteId, user.Balance, user.Cur, user.CreateTime)
	m.Redisclient.MSet("user:lebo:"+user.GUserName, user_v)
	return user, nil
}
...
//加载用户到redis中
func Lebo_LoadAllUserInRedis() error {
	pagesize := 300
	i := 0
	hasuser := true
	for {
		...
		if count > 0 {
			for _, v := range users {
				guser = append(guser, "user:lebo:"+v.GUserName)
