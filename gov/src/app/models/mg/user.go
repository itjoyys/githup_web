...
func CreateUser(user *User) error {
	...
	if err == nil {
		...
		sc := m.Redisclient.MSet("user:mg:"+user.GUserName, user_v)
		if sc.Err() != nil {
...
func GetUserByGname(gnames []string) (map[string]User, error) {
	...
	for _, v := range gnames {
		mgname = append(mgname, "user:mg:"+v)
	}
	users := make([]User, 0)
...
func ChengUserPwd(username, siteid, pwd string) (bool, error) {
	...
	sql := "update mg_user set password = ? where username = ? AND site_id = ?"
...
func GetUserInfo(username, siteid string) (*User, error) {
	...
	user_v := fmt.Sprintf("%v'%v'%v'%v'%v'%v'%v'%v", user.UserName, user.AgentId, user.Password, user.GUserName, user.SiteId, user.Balance, user.Cur, user.CreateTime)
	m.Redisclient.MSet("user:mg:"+user.GUserName, user_v)
	return user, nil
}
...
//加载用户到redis中
func MG_LoadAllUserInRedis() error {
	pagesize := 300
	i := 0
	hasuser := true
	for {
		...
		if count > 0 {
			for _, v := range users {
				guser = append(guser, "user:mg:"+v.GUserName)
