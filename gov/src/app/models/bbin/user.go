...
func CreateUser(user *User) error {
	...
	if err == nil {
		...
		sc := m.Redisclient.MSet("user:bbin:"+user.GUserName, user_v)
		if sc.Err() != nil {
...
func GetUserByGname(gnames []string) (map[string]User, error) {
	...
	for _, v := range gnames {
		mgname = append(mgname, "user:bbin:"+strings.ToLower(v))
	}
	users := make([]User, 0)
...
func GetUserInfo(username, siteid string) (*User, error) {
	...
	m.Redisclient.MSet("user:bbin:"+strings.ToLower(user.GUserName), user_v)
	return user, nil
}
...
//加载用户到redis中
func BBIN_LoadAllUserInRedis() error {
	pagesize := 300
	i := 0
	hasuser := true
	for {
		...
		if count > 0 {
			for _, v := range users {
				guser = append(guser, "user:bbin:"+strings.ToLower(v.GUserName))
...
func ChengUserPwd(username, siteid, pwd string) (bool, error) {
	...
	sql := "update bbin_user set apppwd = ? where username = ? AND site_id = ?"

	_, err := m.Orm.Exec(sql, common.Md5(pwd+siteid), username, siteid)
	if err != nil {
		common.Log.Err("Update BBIN BetRecord :", err)
		return false, err
	}
	return true, nil
}
