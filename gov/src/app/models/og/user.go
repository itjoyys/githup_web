...
func CreateUser(user *User) error {
	...
	if err == nil {
		...
		sc := m.Redisclient.MSet("user:og:"+user.GUserName, user_v)
		if sc.Err() != nil {
			common.Log.Err("Redisclient MSet:", sc.Err(), user)
			m.Redis_user_error[5] = 1
		}
	}
	return err
}
...
func GetUserByGname(gnames []string) (map[string]User, error) {
	...
	for _, v := range gnames {
		mgname = append(mgname, "user:og:"+strings.ToLower(v))
	}
	users := make([]User, 0)
...
func GetUserInfo(username, siteid string) (*User, error) {
	...
	m.Redisclient.MSet("user:og:"+strings.ToLower(user.GUserName), user_v)
	return user, nil
}
...
//加载用户到redis中
func OG_LoadAllUserInRedis() error {
	pagesize := 300
	i := 0
	hasuser := true
	for {
		...
		if count > 0 {
			for _, v := range users {
				guser = append(guser, "user:og:"+strings.ToLower(v.GUserName))
