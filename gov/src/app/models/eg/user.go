...
func CreateUser(user *User) error {
	...
	if err == nil {
		...
		sc := m.Redisclient.MSet("user:eg:"+user.GUserName, user_v)
		if sc.Err() != nil {
...
func GetUserByGname(gnames []string) (map[string]User, error) {
	...
	for _, v := range gnames {
		mgname = append(mgname, "user:eg:"+strings.ToLower(v))
	}
	users := make([]User, 0)
...
func GetUserInfo(username, siteid string) (*User, error) {
	...
	m.Redisclient.MSet("user:eg:"+strings.ToLower(user.GUserName), user_v)
	return user, nil
}
...
//加载用户到redis中
func Eg_LoadAllUserInRedis() error {
	pagesize := 300
	i := 0
	hasuser := true
	for {
		...
		if count > 0 {
			for _, v := range users {
				guser = append(guser, "user:eg:"+strings.ToLower(v.GUserName))
